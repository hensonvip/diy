...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
        ...
        $ecs_template = $db->getOne('SELECT value FROM ' . $ecs->table('mobile_shop_config') . ' WHERE code = \'template\'');
        $style = $db->getOne('SELECT value FROM ' . $ecs->table('mobile_shop_config') . ' WHERE code = \'stylename\'');
        if($style == '')
        {
            $style = '0';
        }
