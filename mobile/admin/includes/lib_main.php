...
function insert_config($parent, $code, $value)
{
    ...
    $sql = 'SELECT id FROM ' . $ecs->table('mobile_shop_config') . " WHERE code = '$parent' AND type = 1";
    $parent_id = $db->getOne($sql);

    $sql = 'INSERT INTO ' . $ecs->table('mobile_shop_config',1) . ' (parent_id, code, value) ' .
...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['ecs']->table('mobile_ad_position');
    $res = $GLOBALS['db']->query($sql);
