...
/*if(isset($_GET['ent_id']) && isset($_GET['ent_ac']) &&  isset($_GET['ent_sign']) && isset($_GET['ent_email']))
{
    ...
    if($token==md5(md5($_CFG['token']).$domain_url.ADMIN_PATH_M))
    {
        ...
        $db->query('UPDATE '.$ecs->table('mobile_shop_config',1) . ' SET value = "'. $ent_id .'" WHERE code = "ent_id"');
        $db->query('UPDATE '.$ecs->table('mobile_shop_config',1) . ' SET value = "'. $ent_ac .'" WHERE code = "ent_ac"');
        $db->query('UPDATE '.$ecs->table('mobile_shop_config',1) . ' SET value = "'. $ent_sign .'" WHERE code = "ent_sign"');
        $db->query('UPDATE '.$ecs->table('mobile_shop_config',1) . ' SET value = "'. $ent_email .'" WHERE code = "ent_email"');
        clear_cache_files();
