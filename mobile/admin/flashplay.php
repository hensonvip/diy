...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    {
        $sql = "UPDATE " .$GLOBALS['ecs']->table('mobile_shop_config',1). " SET value = '$flash_theme' WHERE code = 'flash_theme'";
...
elseif ($_REQUEST['act'] == 'custom_status')
{
    ...
    if (modfiy_ad_status($id, $ad_status))
    {
        ...
        /* 标签初始化 */
        $sql = "SELECT  value FROM " . $ecs->table("mobile_shop_config") . " WHERE id =337";
 ...
 */
 function modfiy_ad_status($ad_id, $ad_status = 0)
 {
    ...
    if ($ad_status == 1)
    {
        ...
        /* 用户自定义广告开启 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table("mobile_shop_config",1) . " SET value = 'cus' WHERE id =337";
        $GLOBALS['db']->query($sql);
    }
    else
    {
        ...
        if ($ad['ad_status'] == 0)
        {
            ...
            {
                $sql = "UPDATE " . $GLOBALS['ecs']->table("mobile_shop_config",1) . " SET value = 'sys' WHERE id =337";
                $GLOBALS['db']->query($sql);
            }
            else
            {
