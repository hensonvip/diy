...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
	...
	$smarty->assign('wap_index_ad',get_wap_advlist2('1', 5));  //wap首页幻灯广告位
    $smarty->assign('wap_index_img',get_wap_advlist2('2', 5));  //wap首页幻灯广告位
    $smarty->assign('wap_index_brand',get_wap_advlist2('3', 1));  //wap首页幻灯广告位
	$smarty->assign('menu_list',get_menu());//图标导航
...
function get_wap_advlist2( $position_id, $num )
{
        $arr = array( );
        $sql = "select ap.ad_width,ap.ad_height,ad.ad_id,ad.ad_name,ad.ad_code,ad.ad_link,ad.ad_id from ".$GLOBALS['ecs']->table( "mobile_ad_position" )." as ap left join ".$GLOBALS['ecs']->table( "mobile_ad" )." as ad on ad.position_id = ap.position_id where ap.position_id='".$position_id.( "' and UNIX_TIMESTAMP()>ad.start_time and UNIX_TIMESTAMP()<ad.end_time and ad.enabled=1 limit ".$num );
        $res = $GLOBALS['db']->getAll( $sql );
        foreach ( $res as $key => $row )
        {
                $arr[$key]['name'] = $row['ad_name'];
                $arr[$key]['url'] = "affiche.php?ad_id=".$row['ad_id']."&uri=".$row['ad_link'];
                $arr[$key]['ad_link'] = $row['ad_link'];
                $arr[$key]['image'] = "data/afficheimg/".$row['ad_code'];
                //$arr[$key]['content'] = "<a href='affiche.php?ad_id=".$row['ad_id']."&uri=".$row['ad_link']."' target='_blank'><img src='data/afficheimg/".$row['ad_code']."' width='".$row['ad_width']."' height='".$row['ad_height']."' /></a>";
...
function get_menu()
{
	$sql = "select * from ".$GLOBALS['ecs']->table('mobile_menu')." order by sort";
