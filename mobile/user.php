...
$ui_arr = array(
	'login','profile','order_list','order_detail','address_list','collection_list','message_list','tag_list',
       ...
       'supplier_reg','comment_order','address','set_address','follow_shop','account_manage','my_comment','shaidan_send','my_history'
);
...
if(in_array($action, $ui_arr))
{
	...
	$sql = "SELECT value FROM " . $ecs->table('mobile_shop_config') . " WHERE id = 419";
	$row = $db->getRow($sql);
	$car_off = $row['value'];
...
function action_account_detail()
{
	...
	// 获取剩余余额
	$surplus_amount = get_user_yue($user_id);
...
function action_my_history(){
	...
	foreach ($history_list as $key => $value) {
		$history_list[$key]['goods_thumb'] = get_pc_url().'/'.get_image_path($value['goods_id'], $value['goods_thumb'], true);
	}
