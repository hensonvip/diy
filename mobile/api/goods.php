...
$hash_code = $db->getOne("SELECT `value` FROM " . $ecs->table('mobile_shop_config') . " WHERE `code`='hash_code'", true);
...
switch ($action)
{
    ...
    case 'get_shop_info':
    {
        ...
        $sql = "SELECT `value` FROM " . $ecs->table('mobile_shop_config') . " WHERE code='shop_name'";
        $shop_name = $db->getOne($sql);
        $sql = "SELECT `value` FROM " . $ecs->table('mobile_shop_config') . " WHERE code='currency_format'";
        $currency_format = $db->getOne($sql);
        $sql = "SELECT r.region_name, sc.value FROM " . $ecs->table('region') . " AS r INNER JOIN " . $ecs->table('mobile_shop_config') . " AS sc ON r.`region_id`=sc.`value` WHERE sc.`code`='shop_country' OR sc.`code`='shop_province' OR sc.`code`='shop_city' ORDER BY sc.`id` ASC";
