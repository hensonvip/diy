...
if ($_REQUEST['act'] == 'tipemail')
{
	...
	if(!$word_hunuo ||  strlen($word_hunuo) > 30)
	{
	...
	$email_name_arr = explode("@", $word_hunuo);
    ...
    {
		if (stristr($email_domain, $word_hunuo))
		{
			$logdb[] = $email_domain;
		}
	}
	...
	if(count($logdb)==0)
	{
		$result_hunuo['content'] = '';
	}
	else
	{		
		$result_hunuo['content'] = $smarty->fetch('library/email_tip.lbi');
	}
