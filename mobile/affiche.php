...
if ($_GET['act'] == 'js')
{
    ...
    $sql = 'SELECT ad.ad_id, ad.ad_name, ad.ad_link, ad.ad_code '.
           'FROM ' . $ecs->table('mobile_ad') . ' AS ad ' .
           'LEFT JOIN ' . $ecs->table('mobile_ad_position') . ' AS p ON ad.position_id = p.position_id '.
...
else
{
    ...
    {
        /* 更新站内广告的点击次数 */
        $db->query('UPDATE ' . $ecs->table('mobile_ad') . " SET click_count = click_count + 1 WHERE ad_id = '$ad_id'");
        ...
        $sql="SELECT * FROM ". $ecs->table('mobile_ad') ." WHERE ad_id = '$ad_id'";
        $ad_info=$db->getRow($sql);
        /* 跳转到广告的链接页面 */
