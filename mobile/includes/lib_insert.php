...
function insert_ecsmart_tel(){
    $sql = "select value from ".$GLOBALS['ecs']->table('mobile_shop_config')." where code = 'service_phone'";
...
function insert_ads($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'FROM ' . $GLOBALS['ecs']->table('mobile_ad') . ' AS a '.
                'LEFT JOIN ' . $GLOBALS['ecs']->table('mobile_ad_position') . ' AS p ON a.position_id = p.position_id ' .
