...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
		...
		include_once('includes/lib_splitword_hunuo.php');
	...
	if($count==0)
	{
		...
		{
			$sql_hunuo = "select keyword from ". $ecs->table('keyword') ." where letter='". $_REQUEST['keywords'] ."' order by items desc limit 0,1";
			$keyword_hunuo = $db->getOne($sql_hunuo);
			if($keyword_hunuo && $keyword_hunuo!=$_REQUEST['keywords'])
			{
				header('Location: search.php?keywords='.$keyword_hunuo.'&keyword_zm='.$_REQUEST['keywords']);
				exit;
			}
		}
	}
	...
	{
		$smarty->assign('beizhuxinxi_hunuo', '关键词<font color=#cc0000>'.$_REQUEST['keyword_zm'].'</font>搜索结果为零，<br>但是我们为您匹配到了相关关键词<font color=#cc0000>'.$_REQUEST['keywords'].'</font>，下面是它的查询结果！');
	}
...
//	$ts_string_array = array("*","/");
    ...
    if($_CFG['fulltext_search'] == '1'){
		...
		if(empty($idarray)){
			$idarray = array();
				foreach($replacef_hunuo as $key => $value){
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['goods_name'] = $row['goods_name'];
		$arr[$row['goods_id']]['goods_name_hunuo'] =  $arr[$row['goods_id']]['goods_name'];
		foreach($replacef_hunuo as $key_hunuo =>$temp_qq)
		{
				//$replacet[$key_hunuo]=  '<strong style="color:#cc0000;">'. $temp_qq .'</strong>';;
				
				if(in_array($temp_qq,$ts_string_array)){
					$arr[$row['goods_id']]['goods_name_hunuo'] = preg_replace('/(?!<[^>]*)\\'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_hunuo']);
				}else{
					$arr[$row['goods_id']]['goods_name_hunuo'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_hunuo']);
				}
