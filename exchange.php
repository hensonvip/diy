...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
		...
		$sql_attr_hunuo_com="SELECT a.attr_id, ga.goods_attr_id FROM ". $GLOBALS['ecs']->table('attribute') . " AS a left join ". $GLOBALS['ecs']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE a.is_attr_gallery=1 and ga.goods_id='" . $goods_id. "' order by ga.goods_attr_id ";
$goods_attr=$GLOBALS['db']->getRow($sql_attr_hunuo_com);
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_gallery_attr')
{
	...
	$gallery_list_hunuo_com=get_goods_gallery_attr_hunuo_com($goods_id, $goods_attr_id);
	$gallery_content=array();
	$gallery_content['thumblist'] ='<ul>';
	foreach($gallery_list_hunuo_com as $gkey=>$gval)
	{
...
die($json->encode($row));
		...
		$return_arr = array();
		foreach ($results_hunuo_com as $value_hunuo)
		{
			$return_arr[$value_hunuo['goods_attr_id']] = $value_hunuo['attr_value'];
		}
		...
		foreach ($properties['spe'] AS $skey_hunuo=>$sval_hunuo)
		{
			$hahaha_zhyh = 0;
			$sskey_hunuo_com = '-1';
			foreach ($sval_hunuo['values'] AS $sskey_hunuo=>$ssval_hunuo)
			{
				if ( is_exist_prod($selected_first, $ssval_hunuo['id'], $prod_exist_arr))
				{
					$hahaha_zhyh = $hahaha_zhyh ? $hahaha_zhyh : $ssval_hunuo['id'];
					$sskey_hunuo_com = ($sskey_hunuo_com != '-1') ? $sskey_hunuo_com : $sskey_hunuo;
				}
				else
				{
					$properties['spe'][$skey_hunuo]['values'][$sskey_hunuo]['disabled'] = "disabled";
				}

				if ($skey_hunuo==$attr_id_gallery)
				{
					$goods_attr_id_qq2211707 = $ssval_hunuo['id'] ;
					...
					$properties['spe'][$skey_hunuo]['values'][$sskey_hunuo]['goods_attr_thumb'] = $db->getOne($sql_qq2211707_qq87139667);
				}
			}
			if ($hahaha_zhyh)
			{
				$selected_first[$skey_hunuo] =  $hahaha_zhyh;
			}
			if ($sskey_hunuo_com!='-1')
			{
				$properties['spe'][$skey_hunuo]['values'][$sskey_hunuo_com]['selected_key_hunuo'] = "1";
			}
		}
		...
		//echo '</pre>';
            ...
            $smarty->assign('pictures',            get_goods_gallery_attr_hunuo_com($goods_id, $goods_attr_id));
 ...
 * @return  array
 */
function get_goods_gallery_attr_hunuo_com($goods_id, $goods_attr_id)
{
