         ...
         if(!$real_goods && $filter['extension_code'] == 'virtual_good'){
             /*$sql = "SELECT distinct(g.goods_id), goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order,supplier_status, goods_number, integral, " .
                    ...
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g,".$GLOBALS['ecs']->table('supplier_goods_cat')." as sgc,".$GLOBALS['ecs']->table('virtual_district')." as dis, ".
                        $GLOBALS['ecs']->table('virtual_goods_district')." as dig".
                        " WHERE dis.supplier_id = ".$_SESSION['supplier_id']." AND sgc.goods_id=g.goods_id and dis.goods_id = g.goods_id and dis.district_id = dig.district_id  AND is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";*/
            //由于隐藏了店铺分类相关的字段不要，所以以上的sql语句有问题，不需要关联查询分类表了，所以搬了主商城后台的查询语句过来用 2017.12.09
            $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
                    ...
                    " WHERE is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
         }else{
            /* $sql = "SELECT distinct(g.goods_id), goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order,supplier_status, goods_number, integral, " .
            ...
            $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
                    ...
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g WHERE is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
         }
