...
if ($_REQUEST['act'] == 'list_edit')
{
	...
	/*$sql="select * from " . $ecs->table('supplier_shop_config') . "  where code='jwdbj' and supplier_id=".$_SESSION['supplier_id'];
	$jwd=$db->getRow($sql);
	if(empty($jwd['id']))
	{ 
		$sql="insert into " . $ecs->table('supplier_shop_config') . " (parent_id, code,type,sort_order,supplier_id) VALUES ('1','jwdbj','jwd','1','".$_SESSION['supplier_id']."')";
     }   */
...
//print_r($group_list);
    ...
    $_CFG['invoice_type']=unserialize($_CFG['invoice_type']);
    ...
    if(empty($supplier_info['latitude']) or empty($supplier_info['longitude'])){//如果坐标为空，则默认北京坐标
        $supplier_info['longitude'] = '116.40355';
        $supplier_info['latitude'] = '39.915156';
    }
...
elseif ($_REQUEST['act'] == 'post')
{
	...
	//$jd= substr($jd,7);
	//$wd= substr($wd,7);
	$sql="select * from " . $ecs->table('supplier_shop_config') . "  where code='jingdu' and supplier_id=".$_SESSION['supplier_id'];
	$jingdu=$db->getRow($sql);
	if(empty($jingdu['id']))
	{ 
		$sql="insert into " . $ecs->table('supplier_shop_config') . " (parent_id, code,type,value,sort_order,supplier_id) VALUES ('1','jingdu','jwd','".$jd."','1','".$_SESSION['supplier_id']."')";
		@$db->query($sql);
	 }else{
		$sql="update " . $ecs->table('supplier_shop_config') . "  set value='".$jd."' where code='jingdu' and supplier_id=".$_SESSION['supplier_id'];
		@$db->query($sql);
	 }   
	
	$sql="select * from " . $ecs->table('supplier_shop_config') . "  where code='weidu' and supplier_id=".$_SESSION['supplier_id'];
	$weidu=$db->getRow($sql);
	if(empty($weidu['id']))
	{ 
		$sql="insert into " . $ecs->table('supplier_shop_config') . " (parent_id, code,type,value,sort_order,supplier_id) VALUES ('1','weidu','jwd','".$wd."','1','".$_SESSION['supplier_id']."')";
		@$db->query($sql);
	 }else{
		$sql="update " . $ecs->table('supplier_shop_config') . "  set value='".$wd."' where code='weidu' and supplier_id=".$_SESSION['supplier_id'];
		@$db->query($sql);
	 }
