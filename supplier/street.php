...
if ($_REQUEST['act'] == 'info')
{
	...
	/* 创建 html editor */
    create_html_editor('FCKeditor1',htmlspecialchars($street_info['content']));
...
}elseif ($_REQUEST['act'] == 'saveinfo')
{
	...
	$sql = "INSERT INTO " .$ecs->table('supplier_street'). " (supplier_id,supplier_type,supplier_name,supplier_title,supplier_desc,content,supplier_tags,add_time,is_show,status)VALUES ".
	       "(".$save['supplier_id'].",".$save['supplier_type'].",'".$save['supplier_name']."','".$save['supplier_title']."','".$save['supplier_desc']."','".$save['content']."','".$save['supplier_tags']."','".$save['addtime']."',1,1) ON DUPLICATE KEY UPDATE ".
	       "supplier_type=".$save['supplier_type'].",supplier_name='".$save['supplier_name']."',supplier_title='".$save['supplier_title']."',supplier_desc='".$save['supplier_desc']."',content='".$save['content']."',supplier_tags='".$save['supplier_tags']."',status=1";
	...
	if($_FILES['logo']['size']){
		...
		if($picinfo){
			...
			$create_pic_info = array('800x474'=>array('width'=>800,'height'=>474));
