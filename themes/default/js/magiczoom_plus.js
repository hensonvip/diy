...
function show_attr_status(theid, goods_id, attr_id_hunuo_com)
{
	...
	var hunuo_com= theid.parentNode.id.replace('catt_', '');
	document.getElementById('spec_attr_type_'+hunuo_com).value=selected_first[0];

	//if(attr_selected()){
	//	return true;
	//}
	if(change_classname(theid,hunuo_com)){
		//var formBuy = document.forms['ECS_FORMBUY'];
		//spec_arr = getSelectedAttributes(formBuy);
		//Ajax.call('goods.php?act=get_products_info', 'id=' + spec_arr+ '&goods_id=' + goods_id, shows_number, 'GET', 'JSON');
		changePriceAll();
	...
	for(zzz=0; zzz<mylist.length; zzz++)
	{
		if(mylist[zzz].className!='wuxiao')
		{
				mylist[zzz].onclick=function(){
					show_attr_status(this, goods_id, attr_id_hunuo_com);
				}
		}
	...
	document.getElementById(the_input_id).checked="checked";
	 ...
	 {
	     selid_hunuo_com=0;
	     if (spec_attr_type[iii].value != hunuo_com)
	     {
		...
		document.getElementById('spec_value_' + selid_hunuo_com).checked = "checked";
		selected_first.push(selid_hunuo_com);
		document.getElementById('xuan_a_'+selid_hunuo_com).className='cattsel';
		//document.getElementById('xuan_a_'+selid_hunuo_com).onclick=function(){}
	     }
	}
	changeAtt(theid, goods_id);
	if (theid.parentNode.id.replace('catt_','')==attr_id_hunuo_com)
	{
 ...
 function is_exist_prod(selected_first, id, prod_exist_arr)
 {
	...
	var hunuo_com_selected = selected_first.slice(0);
	hunuo_com_selected.push(id);
	var all_valid =0;
	for (var i in prod_exist_arr)
	{
		var first_exist=1;
		for (var j in hunuo_com_selected)
		{
			if ( prod_exist_arr[i].indexOf("|" + hunuo_com_selected[j] + "|") =='-1')
			{
				first_exist=0;
				break;
			}
