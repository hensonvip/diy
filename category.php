...
$display  = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display'])  : (isset($_COOKIE['ECS']['display']) ? $_COOKIE['ECS']['display'] : $default_display_type);
	...
	if (!$smarty->is_cached($index_dwt_files, $cache_id))
	{
		...
		$sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ".
				...
				"GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC"; //此SQL语句增加字段 b.brand_logo,  By  www.hunuo.com
		...
		$brand_have_logo = 0;  //代码增加   By    www.hunuo.com
		...
		foreach ($brands AS $key => $val)
		{
			...
			$brands[$temp_key]['brand_id_hunuo'] = $val['brand_id'];
		...
		$smarty->assign('brand_zimu_hunuo', $brand_zimu);
		...
		if ($cat['filter_attr'] > 0)
		{
			...
			foreach ($filter_attr AS $fkey=>$fval)
			{
				if ($fval)
				{
					...
					$condition[]=array(
							...
							'cond_name' =>$cond_name,
							'cond_url' =>build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url_hunuo, 'filter'=>$filter), $cat['cat_name'])
						);
				}
			}
			$attr_group_more_txt = "";
			$attr_group_more_id =0;
			foreach ($all_attr_list AS $k_hunuo=>$k_value)
			{
				if ($k_hunuo >1)
				{
		 ...
		 if($cat['attr_hunuo'])
		 {
			 ...
			 foreach($attr_hunuo as $attr_ecshop68com)
			 {
				if ($attr_ecshop68com)
				 {
    ...
    $smarty->assign('current_cat_pr2_id',$catlist[count($catlist)-2]);/*取得二级分类ID*/
        ...
        //file_put_contents('goodslist.txt',var_export($goodslist,true));
