...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link',  array('text' => $_LANG['originality_not'], 'href' => 'originality_not.php?act=list'));
    ...
    assign_query_info();
    $smarty->display('originality_already_list.htm');
}
...
{
    check_authz_json('originality_already');

    $originality_list = get_originalitylist();
    //var_dump($originality_list);exit;
    $smarty->assign('originality_list',    $originality_list['arr']);
    $smarty->assign('filter',          $originality_list['filter']);
    $smarty->assign('record_count',    $originality_list['record_count']);
    $smarty->assign('page_count',      $originality_list['page_count']);
    ...
    make_json_result($smarty->fetch('originality_already_list.htm'), '',
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    //奖项
    $resql = "SELECT * FROM ".$ecs->table('diy_reward')." WHERE is_show = 1";
    $reward = $db->getAll($resql);
    //print_r($info['reward']);exit;

    $rewards = array();
    foreach($reward as $k=>$v){
        $i = $v['grade'];
        $rewards[$i] =  $v['describe'].':'.$v['prize'];
    }
    //print_r($info['reward']);exit;
    //print_r($info['design_img_t_arr']);exit;
    //投票结束时间 $originality['vote_end_time']
    $originality = get_time();
    
    //当前
    $time = gmtime();
    //print_r($time.'+'.$originality['vote_end_time']);
    ...
    $smarty->assign('ur_here',     $_LANG['originality_list']);
    $smarty->assign('vote_end_time',   $originality['vote_end_time']);
    $smarty->assign('gmtime',   $time);
    //var_dump($rewards);
    $smarty->assign('rewards',   $rewards);
    $smarty->assign('cat_list',   $cat_list);
    $smarty->assign('action_link', array('text' =>  $_LANG['originality_list'], 'href' => 'originality_already.php?act=list&' . list_link_postfix()));
    $smarty->assign('form_action', 'update');

    assign_query_info();
    $smarty->display('originality_already_info.htm');
}
...
if ($_REQUEST['act'] =='update')
{
   ...
   // print_r($_REQUEST);exit;
    if(intval($_POST['record_id'])){
        //$state = $_POST['state'];
        admin_log('作品ID-'.$_POST['record_id'],'edit','originality_already');
        ...
        if(!empty($tags_name)){
            ...
            $tags_name = db_create_in($tags_name);
            //print_r($tags_name);exit;
            $tags_sql = "SELECT `tags_id`,`tags_name` FROM ".$ecs->table('production_tags')." WHERE tags_name ".$tags_name;
            ...
            if($tags_select){
                ...
                if(count($tags_select) <  $name_count){
                    ...
                    foreach ($diff as $k=>$v){
                        $ins_sql = " INSERT INTO ".$ecs->table('production_tags')."( `tags_name`,`addtime`) VALUES ('$v',".time().") ";
                        if((string)$db->query($ins_sql)){
                            $tags_id_arr[] = $db->insert_id();
                        }else{
                            sys_msg('标签格式输入有误，请检查后重试');
                        }
                    }
        ...
        $sql = "UPDATE ".$ecs->table('diy_info')." di ,".$ecs->table('diy_record').
            " dr SET di.title = '$title',di.describe = '$describe',di.type = '$type',di.add_time = '$time',dr.state = '$state',di.tags = '$tags',dr.grade = '".$_POST['cat_id']."' WHERE di.record_id = ".$_POST['record_id']." AND dr.record_id=".$_POST['record_id'];
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if($name_record){
        ...
        if ($exc->drop($id)) {
            ...
            foreach($design_img_t_arr as $k=>$v){
                @unlink(ROOT_PATH.$v);
            }
            ...
            admin_log('作品ID-'.$id,'remove','originality_already');

            clear_cache_files();
        }
    }
    $url = 'originality_already.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch') {
    ...
    if (isset($_POST['type'])) {
        if ($_POST['type'] == 'button_remove')
        {
            ...
            if($db->query($del_sql)){
                ...
                admin_log('作品ID-'.join(',', $_POST['checkboxes']),'batch_remove','originality_already');
            }
            //print_r($img);exit;
        }
    ...
    /* 清除缓存 */
    clear_cache_files();
    $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'originality_already.php?act=list');
    sys_msg($_LANG['batch_handle_ok'], 0, $lnk);
}

elseif($_REQUEST['act'] == 'history_list')
{
    /* 取得过滤条件 */
    $filter = array();
    $smarty->assign('ur_here',      $_LANG['03_originality_already']);
    $smarty->assign('action_link',  array('text' => $_LANG['originality_list'], 'href' => 'originality_list.php?act=list'));
    $smarty->assign('full_page',    1);
    $smarty->assign('filter',       $filter);
...
{
    $originality_id = $_GET['id']?$_GET['id']:0;
    ...
    if ($result === false) {
        ...
        $filter['oid'] =  empty($_REQUEST['oid']) ? $originality_id :trim($_REQUEST['oid']);
    ...
    while($rows = $GLOBALS['db']->fetchRow($res)){
        ...
        $rows['zan_num'] = $GLOBALS['db']->getOne( 'SELECT COUNT(zan_id) FROM '.$GLOBALS['ecs']->table('diy_zan').' WHERE record_id ='. $rows['record_id'] );
        $arr[] = $rows;
    }
...
//获取当前赛季时间
function get_time($originality_id){
    /* 取数据 */
    if($originality_id){
        $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('originality'). " WHERE id = $originality_id";
    }else{
