 ...
 * @return  string
 */
function build_attr_html_layui($cat_id, $goods_id = 0 , $bar_code = 0)
{
    $attr = get_attr_list($cat_id, $goods_id);
    $html = '<table width="100%" id="attrTable">';
    $spec = 0;
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        if ($val['attr_type'] == 1 || $val['attr_type'] == 2)
        {
         ...
         $html.=$val[attr_name]."：</td><td><input type='hidden' name='attr_id_list[]' value='$val[attr_id]' txm='$val[attr_txm]' class='ctxm_$val[attr_txm] layui-input' />";

        if ($val['attr_input_type'] == 0)
        {
            $html .= '<input name="attr_value_list[]" type="text" value="' .htmlspecialchars($val['attr_value']). '" size="40" class="layui-input" /> ';
        }
        elseif ($val['attr_input_type'] == 2)
        {
            $html .= '<textarea name="attr_value_list[]" rows="3" cols="40">' .htmlspecialchars($val['attr_value']). '</textarea>';
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            '<font style="margin-left:15px;">'.$GLOBALS['_LANG']['spec_price'].'</font> <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" class="layui-input" />' :
            ' <input type="hidden" name="attr_price_list[]" value="0" class="layui-input" />';
    ...
    if($bar_code){
        $html .= '<div id="input_txm"><table  width="100%"  >';
        foreach($bar_code as $value){
            $html .='<tr><td class="label">条形码</td><td><input class="layui-input" type="hidden" name="txm_shu[]" value='.$value['taypes'].'>'.$value['taypes'].'<td/><td><input class="layui-input" type="text" name="tiaoxingm[]" value='.$value['bar_code'].'></td></tr>';
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
       ...
       if ($real_goods > -1)
        ...
        if(intval($_REQUEST['supp'])>0){
        	$sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, goods_status, " .
