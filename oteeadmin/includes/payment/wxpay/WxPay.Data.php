...
class WxPayDataBase
{
	...
	**/
	...
	public function MakeSign()
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		// error_log(json_encode($this->values),3,'error_log_sign_values.log');
		
		$string = $this->ToUrlParams();
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".WxPayConfig::KEY;
		// $string = $string . "&key=7810410b5ec17ed00bf2a2e3e095ea9c";
		// error_log(json_encode($string),3,'error_log_sign_str.log');
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
 ...
 *
 */
class WxAppSign extends WxPayDataBase
{
	...
	public function prepayid($value)
	{
...
{	
	...
	**/
	...
	/** -------------------------------------------------------------------------------------- 20170825 Yip -------------------------------------------------------------------------------------------------------------------------- **/
	/**
	* 设置trade_type=MWEB，此参数必传，该字段用于上报支付的场景信息scene_info。 
	* @param string $value 
	**/
	...
	/**
	* 获取trade_type=MWEB，此参数必传，该字段用于上报支付的场景信息scene_info。 的值
	* @return 值
	**/
	...
	/**
	* 判断trade_type=MWEB，此参数必传，该字段用于上报支付的场景信息scene_info。 是否存在
	* @return true 或 false
	**/
