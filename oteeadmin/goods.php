...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    //print_r($suppliers_list_name);exit;
    $suppliers_exists = 0;
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    $smarty->assign('goods_attr_html', build_attr_html_layui($goods['goods_type'], $goods['goods_id'],$bar_code));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_FILES['goods_img']['error'])) // php 4.2 版本才支持 error
    {
        ...
        // 精品图片
        if ($_FILES['best_img']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['best_img']['type']))
            {
                sys_msg($_LANG['invalid_best_img'], 1, array(), false);
            }
        }
        elseif ($_FILES['best_img']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['best_img_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['best_img']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['best_img_too_big'], $htm_maxsize), 1, array(), false);
        }

        // 商品设计图片
        if ($_FILES['design_img']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['design_img']['type']))
            {
                sys_msg($_LANG['invalid_design_img'], 1, array(), false);
            }
        }
        elseif ($_FILES['design_img']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['design_img_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['design_img']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['design_img_too_big'], $htm_maxsize), 1, array(), false);
        }
    }
    /* 4.1版本 */
    else
    {
        ...
        // 精品图片
        if ($_FILES['best_img']['tmp_name'] != 'none')
        {
            if (!$image->check_img_type($_FILES['best_img']['type']))
            {
        ...
        // 商品设计图片
        if ($_FILES['design_img']['tmp_name'] != 'none')
        {
            if (!$image->check_img_type($_FILES['design_img']['type']))
            {
    ...
    $best_img         = '';  // 初始化精品图片
    $design_img       = '';  // 初始化商品设计图片
    $original_img     = '';  // 初始化原始图片
    $old_original_img = '';  // 初始化原始图片旧图

    // 精品图片
    if (isset($_FILES['best_img']) && $_FILES['best_img']['tmp_name'] != '' &&
        isset($_FILES['best_img']['tmp_name']) && $_FILES['best_img']['tmp_name'] != 'none')
    {
        $best_img = $image->upload_image($_FILES['best_img']);
        if ($best_img === false)
        {
    ...
    // 精品图片
    if (isset($_FILES['design_img']) && $_FILES['design_img']['tmp_name'] != '' &&
        isset($_FILES['design_img']['tmp_name']) && $_FILES['design_img']['tmp_name'] != 'none')
    {
        $design_img = $image->upload_image($_FILES['design_img']);
        if ($design_img === false)
        {
            sys_msg($image->error_msg(), 1, array(), false);
        }
    ...
    $goods_total = isset($_POST['goods_total']) ? $_POST['goods_total'] : 0;
    ...
    $best_img = (empty($best_img) && !empty($_POST['best_img_url']) && goods_parse_url($_POST['best_img_url'])) ? htmlspecialchars(trim($_POST['best_img_url'])) : $best_img;

    $design_img = (empty($design_img) && !empty($_POST['design_img_url']) && goods_parse_url($_POST['design_img_url'])) ? htmlspecialchars(trim($_POST['design_img_url'])) : $design_img;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_design, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " .
                    "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date, best_img, design_img, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, goods_total, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, suppliers_id, cost_price, goods_status)" .
                "VALUES ('$_POST[goods_name]', '$_POST[goods_design]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date','$best_img','$design_img','$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number', '$goods_total',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$exclusive', '$suppliers_id', '$cost_price', '$_POST[goods_status]')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_design, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " .
                    "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date,best_img,design_img,goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, goods_total, warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,exclusive, rank_integral, cost_price, goods_status)" .
                "VALUES ('$_POST[goods_name]', '$_POST[goods_design]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date','$best_img', '$design_img', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number','$goods_total',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$exclusive', '$rank_integral', '$cost_price', '$_POST[goods_status]')";
        }
    }
    else
    {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img, original_img, best_img, design_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
        ...
        if ($best_img && $row['best_img'] && !goods_parse_url($row['best_img']))
        {
            @unlink(ROOT_PATH . $row['best_img']);
        }

        if ($design_img && $row['design_img'] && !goods_parse_url($row['design_img']))
        {
            @unlink(ROOT_PATH . $row['design_img']);
        }
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "goods_design = '$_POST[goods_design]', " .
        ...
        if ($best_img)
        {
            $sql .= "best_img = '$best_img', ";
        }
        if ($design_img)
        {
            $sql .= "design_img = '$design_img', ";
        }
        if ($code != '')
        {
...
//-- 待审核商品列表
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'check_list')
{
    admin_priv('goods_manage');
    ...
    $where = "is_delete='0' AND goods_status = 2";
    /* 关键字 */
    ...
    assign_query_info();
    $smarty->display('goods_list_check.htm');
}
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    if(!$type || !$id || ($type != 'yes' && $type != 'no')){
        sys_msg("缺少必要参数");
    }

    switch ($type) {
        case 'yes':
            $update = 'goods_status = 4, is_on_sale = 1';
            break;
        case 'no':
            $update = 'goods_status = 3, is_on_sale = 0';
            break;
    }
    $GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('goods')." SET ".$update." WHERE goods_id = ".$id);
...
//-- 切换商品类型 使用layui样式
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'get_attr_layui')
{
    check_authz_json('goods_manage');
    ...
    $content    = build_attr_html_layui($goods_type, $goods_id,$bar_code);
