...
if ($_REQUEST['act'] == 'list')
{
    /* 取得过滤条件 */
    $filter = array();
    $smarty->assign('ur_here',      $_LANG['originality_not']);
    ...
    assign_query_info();
    $smarty->display('originality_not_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('originality_not_list.htm'), '',
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    assign_query_info();
    $smarty->display('originality_not_info.htm');
}
...
elseif ($_REQUEST['act'] == 'batch') {
    ...
    /* 清除缓存 */
    clear_cache_files();
    $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'originality_not.php?act=list');
...
function get_originalitylist()
{
    ...
    if ($result === false) {
        ...
        /* 列表总数 */
        $sql = 'SELECT count(*) '.
               'FROM ' .$GLOBALS['ecs']->table('diy_record'). 'dr LEFT JOIN '.$GLOBALS['ecs']->table('diy_info').' di ON dr.record_id = di.record_id WHERE dr.state = 1 AND dr.add_time >= '.$collect_start_time. $where;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $filter = page_and_size($filter);

        /* 获取列表数据 */
        $sql = 'SELECT dr.record_id as record_id,di.describe as `describe`,di.title as `title`,di.design_img as `img`,di.add_time as add_time,dr.state as state '.
               'FROM ' .$GLOBALS['ecs']->table('diy_record').
            ' dr LEFT JOIN '.$GLOBALS['ecs']->table('diy_info').' di ON dr.record_id = di.record_id WHERE dr.state = 1 AND dr.add_time >= '
            .$collect_start_time. $where;
