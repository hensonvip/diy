...
$exc   = new exchange($ecs->table("mobile_ad"), $db, 'ad_id', 'ad_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('mobile_ad'). " WHERE ad_name = '$ad_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$ecs->table('mobile_ad'). " (position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
    ...
    $link[0]['href'] = 'mads.php?act=list';

    $link[1]['text'] = $_LANG['continue_add_ad'];
    $link[1]['href'] = 'mads.php?act=add';
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告数据 */
    $sql = "SELECT * FROM " .$ecs->table('mobile_ad'). " WHERE ad_id='".intval($_REQUEST['id'])."'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($type == 0)
    {
        if ((isset($_FILES['ad_img']['error']) && $_FILES['ad_img']['error'] == 0) || (!isset($_FILES['ad_img']['error']) && isset($_FILES['ad_img']['tmp_name']) && $_FILES['ad_img']['tmp_name'] != 'none'))
        {
            ...
            // 删除旧图片
            $sql = "SELECT ad_code FROM " .$GLOBALS['ecs']->table('mobile_ad'). " WHERE ad_id = '".$id."'";
            $logo_name = $db->getOne($sql);
            if (!empty($logo_name))
            {
                @unlink('../' . DATA_DIR . '/afficheimg/' .$logo_name);
            }
        }
        else
        {
    ...
    /* 更新信息 */
    $sql = "UPDATE " .$ecs->table('mobile_ad'). " SET ".
...
function get_adslist()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('mobile_ad'). ' AS ad ' . $where;
    ...
    $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name '.
            'FROM ' .$GLOBALS['ecs']->table('mobile_ad'). 'AS ad ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('mobile_ad_position'). ' AS p ON p.position_id = ad.position_id '.
