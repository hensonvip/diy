...
if ($_REQUEST['act'] == 'restore')
{
    ...
    else
    {
        ...
        foreach ($real_list AS $file)
        {
            ...
            $add_time = date('Y-m-d H:i:s',strtotime($info['date']) + 28800);
            $list[] = array('name' => $file, 'ver' => $info['ecs_ver'], 'add_time' => $add_time, 'vol' => $info['vol'], 'file_size' => num_bitunit($file_size), 'mark' => $mark);
        }
    }
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    /* 初始化输入变量 */
    $sql_name = filter_var($_REQUEST['sql_file_name'],FILTER_SANITIZE_STRING);
    if (empty($sql_name))
    {
    ...
    {
        $sql_file_name = str_replace("0xa", '', trim($sql_name)); // 过滤 0xa 非法字符
