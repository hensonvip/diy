...
elseif ($_REQUEST['act'] == 'act_export_excel')
{
	admin_priv('users_manage');
	 ...
	 $sql_hunuo = "select * from ". $GLOBALS['ecs']->table('user_rank');
	 $res_hunuo = $GLOBALS['db']->query($sql_hunuo);
	 while ($row_hunuo = $GLOBALS['db']-> fetchRow($res_hunuo))
	 {
		 if ($row_hunuo['special_rank'])
		 {
			$rank_list_www_com[$row_hunuo['rank_id']] = $row_hunuo['rank_name'];
		 }
		 else
		 {
			$rank_list_www_com[0][$row_hunuo['rank_id']] = array(
																							'rank_name' => $row_hunuo['rank_name'],
																							'min_points' => $row_hunuo['min_points'],
																							'max_points' => $row_hunuo['max_points']
																							);
		 }
	 }
