...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if ($is_add)
    {
        $favourable = array(
            'act_id'        => 0,
            'act_name'      => '',
            'start_time'    => local_date('Y-m-d H:i',gmtime()),
            'end_time'      => local_date('Y-m-d H:i',gmtime()+86400*7),
            'user_rank'     => '',
            'act_range'     => FAR_ALL,
            'act_range_ext' => '',
            'min_amount'    => 0,
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $favourable = array(
        ...
        'gift'          => serialize($gift),
        'ext_info'   => serialize(array('price_ladder' => $price_ladder))
    );
...
elseif ($_REQUEST['act'] == 'search')
{
    ...
    {
        $sql = "SELECT goods_id AS id, goods_name AS name FROM " . $ecs->table('goods') .
            " WHERE is_delete = 0  AND is_on_sale = 1 AND supplier_id=0 AND (goods_name LIKE '%" . mysql_like_quote($filter->keyword) . "%'" .
