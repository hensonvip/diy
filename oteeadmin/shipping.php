...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    if ($id > 0)
    {
		...
		$db->query("UPDATE " .$ecs->table('shipping'). " SET enabled = 1,shipping_type=".$shipping_type." WHERE shipping_code = '$_GET[code]' and supplier_id=0 LIMIT 1");
    }
    else
    {
        ...
        $sql = "INSERT INTO " . $ecs->table('shipping') . " (" .
                    "shipping_code, shipping_name, shipping_desc, insure, support_cod, enabled, print_bg, config_lable, print_model , support_pickup , is_default_show,shipping_type" .
                ") VALUES (" .
                    "'" . addslashes($modules[0]['code']). "', '" . addslashes($_LANG[$modules[0]['code']]) . "', '" .
                    addslashes($_LANG[$modules[0]['desc']]) . "', '$insure', '" . intval($modules[0]['cod']) . "', 1, '" . addslashes($modules[0]['print_bg']) . "', '" . addslashes($modules[0]['config_lable']) . "', '" . $modules[0]['print_model'] . "', $support_pickup, $is_default_show, $shipping_type)";
        $db->query($sql);
        $id = $db->insert_Id();
    }
