...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
	...
	$delivery_id = $GLOBALS['db']->getOne("select delivery_id from ".$GLOBALS['ecs']->table('delivery_order')."where order_id=".$order_id);
	if (!empty($delivery_id))
	{
		update_kuaidi_order($delivery_id);
	}
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('invoice' == $step)
    {
        ...
        //更新订单快递信息
        $sql = "UPDATE " . $ecs->table('delivery_order') ." SET invoice_no = '$invoice_no', shipping_name = '".addslashes($shipping['shipping_name'])."' " ."WHERE order_id = '$order_id'";
        $db->query($sql);
...
//                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card')
                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card' || $value['extension_code'] == 'pre_sale' || $value['extension_code'] == 'exchange_goods')
                    {
...
//            sys_msg($_LANG['act_false'], 1, $links);
    ...
    elseif ('receive' == $operation)
    {
        ...
        $arr['shipping_time_end'] = gmtime();//2017.08.04 修复添加记录收货时间
        update_order($order_id, $arr);
...
function order_list()
{
    ...
    foreach ($row AS $key => $value)
    {
        ...
        /*添加订单类型显示 2018.02.01*/
        $row[$key]['order_type_status'] = '';
        if($value['extension_code'] == ''){
        ...
        if($value['extension_code'] == 'bargain'){
            $order_type_name = '砍价订单';
        }
        if($value['extension_code'] == 'group'){
            $order_type_name = '拼团订单';
            $is_finish = $GLOBALS['db']->getOne("SELECT is_finish FROM " . $GLOBALS['ecs']->table('group_log') . " WHERE order_id = '$value[order_id]' ");
            switch ($is_finish) {
                case '0':
                    $row[$key]['order_type_status'] = '<font color="green">进行中</font>';
                    break;
                case '1':
                    $row[$key]['order_type_status'] = '<font color="green">已完成</font>';
                    break;
                case '2':
                    $row[$key]['order_type_status'] = '<font color="red">拼团失败未退款</font>';
                    break;
                case '3':
                    $row[$key]['order_type_status'] = '<font color="red">拼团失败并退钱到余额</font>';
                    break;
                case '4':
                    $row[$key]['order_type_status'] = '<font color="red">拼团失败</font><br/><font color="green">自动原路退回支付账户成功</font>';
                    break;
                case '5':
                    $row[$key]['order_type_status'] = '<font color="red">拼团失败</font><br/><font color="red">自动原路退回支付账户失败</font>';
                    break;
                default:
                    # code...
                    break;
        ...
        $row[$key]['order_type_name'] = $order_type_name;
