...
elseif ($_REQUEST['act'] == 'add')
{
	...
	$smarty->assign('goods_cat_list', cat_list_to_json($cat_list, $brand['cat_id']));
	$smarty->assign('goods_cat_name', $cat_list[$brand['cat_id']]['cat_name']);
	$smarty->assign('goods_cat_id', $brand['cat_id']);
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, cat_id, site_url, brand_desc, brand_logo, is_show, sort_order) ".
           "VALUES ('$_POST[brand_name]', '$_POST[cat_id]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '$_POST[sort_order]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id,cat_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    /* 处理图片 */
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $param = "brand_name = '$_POST[brand_name]',  cat_id = '$_POST[cat_id]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]' ";
    if (!empty($img_name))
    {
        //有图片上传
        ...
        // 删除品牌的旧图标
        $sql = "SELECT brand_logo FROM " .$GLOBALS['ecs']->table('brand'). " WHERE brand_id = '".$_POST['id']."'";
