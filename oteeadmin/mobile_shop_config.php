...
if ($_REQUEST['act'] == 'list_edit')
{
	...
	/* 取得用户等级 */
	$sql = "SELECT value FROM " . $ecs->table('mobile_shop_config') . " WHERE code = 'distrib_rank'";
	$ranks = $db->getOne($sql);
...
elseif ($_REQUEST['act'] == 'post')
{
    ...
    $arr = array();
    $sql = 'SELECT id, value FROM ' . $ecs->table('mobile_shop_config',1);
    $res= $db->query($sql);
    while($row = $db->fetchRow($res))
    {
    ...
    foreach ($_POST['value'] AS $key => $val)
    {
        if($arr[$key] != $val)
        {
            $sql = "UPDATE " . $ecs->table('mobile_shop_config',1) . " SET value = '" . trim($val) . "' WHERE id = '" . $key . "'";
            $db->query($sql);
        }
    }
    ...
    /* 处理上传文件 */
    $file_var_list = array();
    $sql = "SELECT * FROM " . $ecs->table('mobile_shop_config',1) . " WHERE parent_id > 0 AND type = 'file'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    foreach ($_FILES AS $code => $file)
    {
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                {
                    $sql = "UPDATE " . $ecs->table('mobile_shop_config',1) . " SET value = '$file_name' WHERE code = '$code'";
                    $db->query($sql);
                }
                else
                {
	...
	{
		$sql = "UPDATE " . $ecs->table('mobile_shop_config',1) . " SET value = '" . $_POST['all_rank'][0] . "' WHERE code = 'distrib_rank'";
        $db->query($sql);
	}
	else
	{
		if(!empty($_POST['user_rank']))
		{
			...
			$sql = "UPDATE " . $ecs->table('mobile_shop_config',1) . " SET value = '" . $user_rank . "' WHERE code = 'distrib_rank'";
			$db->query($sql);
		}
        else
        {
            $sql = "UPDATE " . $ecs->table('mobile_shop_config',1) . " SET value = '0' WHERE code = 'distrib_rank'";
            $db->query($sql);
        }
	}
	//dqy add end 2015-7-7
    ...
    if (!empty($_POST['invoice_rate']))
    {
        ...
        $sql = "UPDATE " . $ecs->table('mobile_shop_config',1) . " SET value = '" . serialize($invoice) . "' WHERE code = 'invoice_type'";
        $db->query($sql);
    }
...
function update_configure($key, $val='')
{
    if (!empty($key))
    {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('mobile_shop_config',1) . " SET value='$val' WHERE code='$key'";
...
function get_settings($groups=null, $excludes=null)
{
    ...
    /* 取出全部数据：分组和变量 */
    $sql = "SELECT * FROM " . $ecs->table('mobile_shop_config',1) .
