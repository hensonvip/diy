...
elseif ($_REQUEST['act'] == 'batch_start')
{
    ...
    $id_arr = filter_var_array($_POST['checkboxes'], FILTER_SANITIZE_SPECIAL_CHARS);
    if (!isset($id_arr) || !is_array($id_arr))
    {
        sys_msg($_LANG['no_select_goods'], 1);
    }
    $date = remove_xss($_POST['date']);
    if($date == '0000-00-00')
    {
        $date = 0;
    }
    ...
    {
        $db->autoReplace($ecs->table('auto_manage'), array('item_id' => intval($id),'type' => 'article',
            'starttime' => $date), array('starttime' =>(string)$date));
    }
...
elseif ($_REQUEST['act'] == 'batch_end')
{
    ...
    {
        $db->autoReplace($ecs->table('auto_manage'), array('item_id' => intval($id),'type' => 'article',
            'endtime' => $date), array('endtime' =>(string)$date));
    }
