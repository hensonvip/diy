...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 检查销量的上下限是否合理 */
    if ($_POST['min_sale_number'] >= $_POST['max_sale_number'])
    {
        sys_msg('销量上限必须大于销量下限。', 1);
    }
    /* 检查销量下限有无重复 */
    if (!$exc->is_only('min_sale_number', intval($_POST['min_sale_number'])))
    {
        sys_msg("已经存在一个等级积分下限为 " . intval($_POST['min_sale_number']) . " 的会员等级");
    }
    /* 检查销量上限有无重复 */
    if (!$exc->is_only('max_sale_number', intval($_POST['max_sale_number'])))
    {
        sys_msg("已经存在一个等级积分上限为 " . intval($_POST['max_sale_number']) . " 的会员等级");
    }
	...
	$sql = "INSERT INTO " .$ecs->table('user_rank') ."( ".
                "rank_name, min_points, max_points, discount, special_rank,is_recomm, show_price, min_sale_number, max_sale_number, commision_scale1, commision_scale2, sale_number, sale_price".
            ") VALUES (".
                "'$_POST[rank_name]', '" . intval($_POST['min_points']) . "', '" . intval($_POST['max_points']) . "', ".
                "'$_POST[discount]', '$special_rank', $recomm,'" . intval($_POST['show_price']) . "', '" . intval($_POST['min_sale_number']) . "', '" . intval($_POST['max_sale_number']) . "', '" . intval($_POST['commision_scale1']) . "', '" . intval($_POST['commision_scale2']) . "', '" . intval($_POST['sale_number']) . "', '" . intval($_POST['sale_price']) . "')";
    $db->query($sql);
 ...
 *  ajax编辑销量下限
 */
elseif ($_REQUEST['act'] == 'edit_min_sale_number')
{
    check_authz_json('user_rank');
    ...
    $rank = $db->getRow("SELECT max_sale_number, special_rank FROM " . $ecs->table('user_rank') . " WHERE rank_id = '$rank_id'");
    if ($val >= $rank['max_sale_number'] && $rank['special_rank'] == 0)
    {
        make_json_error('销量上限必须大于销量下限。');
    }

    if ($rank['special_rank'] ==0 && !$exc->is_only('min_sale_number', $val, $rank_id))
    {
        make_json_error("已经存在一个销量下限为 {$val} 的会员等级");
    }

    if ($exc->edit("min_sale_number = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
        admin_log(addslashes($rank_name), 'edit', 'user_rank');
        make_json_result($val);
 ...
 *  ajax修改销量上限
 */
elseif ($_REQUEST['act'] == 'edit_max_sale_number')
{
     check_authz_json('user_rank');
    ...
    $rank = $db->getRow("SELECT min_sale_number, special_rank FROM " . $ecs->table('user_rank') . " WHERE rank_id = '$rank_id'");

    if ($val <= $rank['min_sale_number'] && $rank['special_rank'] == 0)
    {
        make_json_error('销量上限必须大于销量下限。');
    }

    if ($rank['special_rank'] ==0 && !$exc->is_only('max_sale_number', $val, $rank_id))
    {
        make_json_error("已经存在一个等级积分上限为 {$val} 的会员等级");
    }
    if ($exc->edit("max_sale_number = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
        admin_log(addslashes($rank_name), 'edit', 'user_rank');
        make_json_result($val);
 ...
 *  修改第1阶段佣金比例
 */
elseif ($_REQUEST['act'] == 'edit_commision_scale1')
{
    check_authz_json('user_rank');
    ...
    if ($val < 1 || $val > 100)
    {
        make_json_error("您没有输入佣金比例或者佣金比例无效。");
    }

    if ($exc->edit("commision_scale1 = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
         admin_log(addslashes($rank_name), 'edit', 'user_rank');
         clear_cache_files();
 ...
 *  修改第2阶段佣金比例
 */
elseif ($_REQUEST['act'] == 'edit_commision_scale2')
{
    check_authz_json('user_rank');
    ...
    if ($exc->edit("commision_scale2 = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
         admin_log(addslashes($rank_name), 'edit', 'user_rank');
         clear_cache_files();
 ...
 *  修改可销售数量
 */
elseif ($_REQUEST['act'] == 'edit_sale_number')
{
    check_authz_json('user_rank');
    ...
    {
        make_json_error("您没有输入销售数量或者销售数量无效。");
    }

    if ($exc->edit("sale_number = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
         admin_log(addslashes($rank_name), 'edit', 'user_rank');
         clear_cache_files();
 ...
 *  修改售价
 */
elseif ($_REQUEST['act'] == 'edit_sale_price')
{
    check_authz_json('user_rank');
    ...
    if (!is_numeric($val) || $val < 1)
    {
        make_json_error("您没有输入售价或者售价无效。");
    }

    if ($exc->edit("sale_price = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
         admin_log(addslashes($rank_name), 'edit', 'user_rank');
         clear_cache_files();
