...
if ($_REQUEST['act'] == 'insert')
{
 ...
 $cat['filter_attr']  = !empty($_POST['filter_attr'])  ? implode(',', array_unique(array_diff($_POST['filter_attr'],array(0)))) : 0;
    ...
    // 分类图片 add by qinglin 2017.09.30
    $cat['type_img'] = basename($image->upload_image($_FILES['type_img'],'categorylogo'));
...
if ($_REQUEST['act'] == 'update')
{
	...
	// 分类图片 add by qinglin 2017.09.30
    $type_img = basename($image->upload_image($_FILES['type_img'],'categorylogo'));
    if (!empty($type_img))
    {
        //有图片上传
        ...
        // 删除品牌的旧图标
        $sql = "SELECT type_img FROM " .$GLOBALS['ecs']->table('category'). " WHERE cat_id = '".$cat_id."'";
...
if ($_REQUEST['act'] == 'drop_logo')
{
    ...
    $link= array(array('text' => '返回编辑分类', 'href' => 'category.php?act=edit&cat_id=' . $cat_id), array('text' => '分类列表', 'href' => 'category.php?act=list'));
    sys_msg('删除分类图片删除', 0, $link);
}
