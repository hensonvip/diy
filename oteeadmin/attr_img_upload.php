...
if($_REQUEST['goods_attr_id'] == 0){
    ...
    $attrs_arr = explode("-", trim($_REQUEST['goods_attr_id']));
    $goods_attr_id = intval($attrs_arr[0]) >0 ? intval($attrs_arr[0]) : '-1';
    $goods_attr_id2 = intval($attrs_arr[1]) >0 ? intval($attrs_arr[1]) : '-1';
}
if(!$goods_id or $goods_attr_id=='-1' or $goods_attr_id2=='-1')
{
...
if($_REQUEST['act']=="upload")
{
    ...
    $array = handle_gallery_image_attr($goods_id, $goods_attr_id,$goods_attr_id2, $file);
...
$img_list =array();
$sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_attr_id= '$goods_attr_id' and goods_attr_id2= '$goods_attr_id2' and goods_id = '$goods_id' order by img_sort,img_id ";
...
if ($img_list)
{
	foreach($img_list as $img)
	{
		...
		echo '<br><br><a href="?act=drop_image' . '&img_id='. $img['img_id'] .'" onclick="javascript: return  (confirm(\'确认删除此图片吗\'))">[删除]</a> <a href="?act=set_attrimage&goods_id=' . $goods_id . '&goods_attr_id=' . $goods_attr_id."-".$goods_attr_id2 . '&img_id='. $img['img_id'] .'">[属性图片]</a><br>顺序:<input type="text" name="sort" value="'.$img['img_sort'].'" style="width:20px;" onblur="set_sort(this.value,'.$img['img_id'].')">';
		echo '</div>';
	}
}
else
...
<input type="hidden" name="goods_attr_id" id="goods_attr_id" value="<?php echo $goods_attr_id.'-'.$goods_attr_id2;?>">
...
$('#test').diyUpload({
	url:'attr_img_upload.php?goods_id=<?php echo $goods_id;?>&goods_attr_id=<?php echo $goods_attr_id."-".$goods_attr_id2;?>&act=upload',
 ...
 * @return  void
 */
function handle_gallery_image_attr($goods_id, $goods_attr_id,$goods_attr_id2, $image_files)
{
    /* 是否处理缩略图 */
    $proc_thumb = (isset($GLOBALS['shop_id']) && $GLOBALS['shop_id'] > 0)? false : true;
        ...
        if ($flag)
        {
           ...
           $img_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'] , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
            ...
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original, goods_attr_id,goods_attr_id2) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original', '$goods_attr_id','$goods_attr_id2')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
