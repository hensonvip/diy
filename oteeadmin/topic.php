...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    switch ($topic_type)
    {
        ...
        case '1':
            ...
            {
                /* 来自互联网图片 不可以是服务器地址 */
                if(strstr($url, 'http') && !strstr($url, $_SERVER['SERVER_NAME']))
                {
                    /* 取互联网图片至本地 */
                    $topic_img = get_url_image($url);
                }
                else{
    ...
    // 标题图上传
    $title_url = filter_var($_REQUEST['title_url'],FILTER_SANITIZE_STRING);
    ...
    else if (!empty($title_url))
    {
        /* 来自互联网图片 不可以是服务器地址 */
        if(strstr($title_url, 'http') && !strstr($title_url, $_SERVER['SERVER_NAME']))
        {
            /* 取互联网图片至本地 */
            $title_pic = get_url_image($title_url);
        }
        else{
            sys_msg($_LANG['web_url_no']);
        }
