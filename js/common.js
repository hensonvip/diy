...
function isSelectAttr(spec_arr) {
    ...
    //已选规格个数
    if(spec_arr){
    	var spec_str = ""+spec_arr+"";
		var spec_array = spec_str.split(",");
		var spec_num=spec_array.length;
    }
    else{
    	var spec_num=0;
    }
...
function addToCartResponse(result) {
	...
	if (result.error > 0) {
		...
		var cart_url = 'flow.php?step=one_cart';
		if (cartInfo) {
 ...
 * 预售添加商品到收藏夹
 */
function pre_collect(goodsId) {
	Ajax.call('user.php?act=collect', 'id=' + goodsId, pre_collectResponse, 'GET', 'JSON');
}

/*******************************************************************************
 * 处理收藏商品的反馈信息
 */
function pre_collectResponse(result) {
	if (result.error > 0) {
		var text='该商品已经存在于您的收藏夹中。';
		alert(result.message);
	} else {
