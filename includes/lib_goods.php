 ...
 * @return  array
 */
function get_goods_properties($goods_id, $goods_status = 0)
{
    /* 对属性进行重新排序和分组 */
    ...
    // diy商品已出售
    if ($goods_status == GS_PASS) {
        /* 获得商品的规格 */
        $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                    "g.goods_attr_id, g.attr_value, g.attr_price " .
                'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
                'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
                "WHERE g.goods_id = '$goods_id' AND g.is_sale = 1 " .
    ...
    foreach ($res AS $row)
    {
        ...
        else
        {
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
                ...
                'default_icon' => $GLOBALS['db']->getOne("SELECT default_icon FROM " . $GLOBALS['ecs']->table('attribute_icon') . " WHERE attr_id = '$row[attr_id]' AND attr_value_name = '$row[attr_value]'"),
                'select_icon'  => $GLOBALS['db']->getOne("SELECT select_icon FROM " . $GLOBALS['ecs']->table('attribute_icon') . " WHERE attr_id = '$row[attr_id]' AND attr_value_name = '$row[attr_value]'"),
                'color_code'  => $GLOBALS['db']->getOne("SELECT color_code FROM " . $GLOBALS['ecs']->table('attribute_color') . " WHERE attr_id = '$row[attr_id]' AND color_name = '$row[attr_value]'"),
            );
        }
...
function favourable_info($act_id)
{
    ...
    if (!empty($row))
    {
        ...
        //新增多个现金减免功能，新增ext_info字段 by qinglin 2018.3.1
...
//"SELECT SUM(og.`goods_number`)  AS num   FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og , " . $GLOBALS['ecs']->table('order_info') . " AS oi WHERE oi.pay_status = 2  AND oi.order_status >= 1  AND oi.order_id = og.order_id AND og.goods_id = " . $goods_id;
