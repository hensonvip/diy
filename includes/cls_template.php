...
class cls_template
{
    ...
    function get_val($val)
    {
        if (strrpos($val, '[') !== false)
        {
            ...
            $val = preg_replace_callback("/\[([^\[\]]*)\]/", function($r) {return '.'.str_replace('$','$',$r[1]);}, $val);//修饰符 /e 在 PHP5.5.x 中已经被弃用了
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            $pattern = '/<!--\s#BeginLibraryItem\s\"\/(.*?)\"\s-->.*?<!--\s#EndLibraryItem\s-->/s';//修饰符 /e 在 PHP5.5.x 中已经被弃用了
