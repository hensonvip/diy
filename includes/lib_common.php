...
function is_mobile_phone ($mobile_phone)
{
    ...
    return preg_match('#^13[\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^17[0,3,6,7,8]{1}\d{8}$|^18[\d]{9}$#', $mobile_phone) ? true : false;
}
...
function is_tel ($tel)
{
     return preg_match('#^([0-9]{3,4}-)?[0-9]{7,8}$#', $tel) ? true : false;
}
        ...
        foreach ($res as $rows)
        {
			...
			$bb = unserialize($rows['gift']);
				...
				case 0:
					...
					break;
				case 1:
					$favourable[$rows['act_id']]['act_type'] = "满减";
					break;
				case 2:
 ...
 * @return void
 */
function get_font_children ($type_id = 0)
{
    return db_create_in(array_unique(array_merge(array($type_id), array_keys(font_type_list($type_id, 0, false)))), 'type_id');
}
...
//复制上面函数，用于接口
function virtual_goods_ship_api(&$virtual_goods, &$msg, $order_sn, $return_result = false, $process = 'other')
{
 ...
 *
 * @access  public
 * @param   int     $type_id     分类的ID
 * @param   int     $selected   当前选中分类的ID
 * @param   boolean $re_type    返回的类型: 值为真时返回下拉列表,否则返回数组
 * @param   int     $level      限定返回的级数。为0时返回所有级数
 * @return  mix
 */
function font_type_list($type_id = 0, $selected = 0, $re_type = true, $level = 0)
{
    static $res = NULL;

    if ($res === NULL)
    {
        $data = read_static_cache('font_type_pid_releate');
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.type_id) AS has_children, COUNT(a.font_id) AS font_num ".
               ' FROM ' . $GLOBALS['ecs']->table('font_type') . " AS c".
               " LEFT JOIN " . $GLOBALS['ecs']->table('font_type') . " AS s ON s.parent_id=c.type_id".
               " LEFT JOIN " . $GLOBALS['ecs']->table('font') . " AS a ON a.type_id=c.type_id".
               " GROUP BY c.type_id ".
               " ORDER BY parent_id, sort_order, type_id ASC";
            $res = $GLOBALS['db']->getAll($sql);
            write_static_cache('font_type_pid_releate', $res);
        }
        else
        {
            $res = $data;
    ...
    $options = font_type_options($type_id, $res); // 获得指定分类下的子分类的数组
    ...
    foreach ($options AS $key => $value)
    {
        ...
        if ($pre_key > 0)
        {
            if ($options[$pre_key]['type_id'] == $options[$key]['parent_id'])
            {
                $options[$pre_key]['has_children'] = 1;
            }
        }
    ...
    if ($re_type == true)
    {
        ...
        foreach ($options AS $var)
        {
            ...
            $select .= ' type_flag="' . $var['type_flag'] . '" ';
            ...
            $select .= htmlspecialchars(addslashes($var['type_name'])) . '</option>';
        }
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('font_type', array('acid' => $value['type_id']), $value['type_name']);
        }
        return $options;
    }
}
...
/**
 * 过滤和排序所有字体分类，返回一个带有缩进级别的数组
 *
 * @access  private
 * @param   int     $type_id     上级分类ID
 * @param   array   $arr        含有所有分类的数组
 * @param   int     $level      级别
 * @return  void
 */
function font_type_options($spec_type_id, $arr)
{
    static $type_options = array();

    if (isset($type_options[$spec_type_id]))
    {
        return $type_options[$spec_type_id];
    }

    if (!isset($type_options[0]))
    {
        $level = $last_type_id = 0;
        $options = $type_id_array = $level_array = array();
        while (!empty($arr))
        {
            foreach ($arr AS $key => $value)
            {
                $type_id = $value['type_id'];
                if ($level == 0 && $last_type_id == 0)
                {
                    if ($value['parent_id'] > 0)
                    {
                        break;
                    ...
                    $options[$type_id]          = $value;
                    $options[$type_id]['level'] = $level;
                    $options[$type_id]['id']    = $type_id;
                    $options[$type_id]['name']  = $value['type_name'];
                    unset($arr[$key]);
                    ...
                    $type_id_array = array($type_id);
                    $level_array[$last_type_id] = ++$level;
                    continue;
                }

                if ($value['parent_id'] == $last_type_id)
                {
                    $options[$type_id]          = $value;
                    $options[$type_id]['level'] = $level;
                    $options[$type_id]['id']    = $type_id;
                    ...
                    if ($value['has_children'] > 0)
                    {
                        if (end($type_id_array) != $last_type_id)
                        {
                            $type_id_array[] = $last_type_id;
                        }
                        $last_type_id    = $type_id;
                ...
                elseif ($value['parent_id'] > $last_type_id)
                {
                    break;
                }
            }
            ...
            if ($count > 1)
            {
                $last_type_id = array_pop($type_id_array);
            }
            elseif ($count == 1)
            {
                if ($last_type_id != end($type_id_array))
                {
                    $last_type_id = end($type_id_array);
                }
                else
                {
                    $level = 0;
            ...
            if ($last_type_id && isset($level_array[$last_type_id]))
            {
                $level = $level_array[$last_type_id];
            }
            else
            {
                $level = 0;
        ...
        $type_options[0] = $options;
    }
    else
    {
        $options = $type_options[0];
    }
    ...
    {
        if (empty($options[$spec_type_id]))
        {
            return array();
        }

        $spec_type_id_level = $options[$spec_type_id]['level'];
        ...
        $spec_type_id_array = array();
        foreach ($options AS $key => $value)
        {
            if (($spec_type_id_level == $value['level'] && $value['type_id'] != $spec_cat_id) ||
                ($spec_type_id_level > $value['level']))
            {
                break;
            }
            else
            {
                $spec_type_id_array[$key] = $value;
            }
        }
        $type_options[$spec_type_id] = $spec_type_id_array;

        return $spec_type_id_array;
    }
}
 ...
 * @return  商品最终购买价格
 */
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array(), $user_rank_info = array(), $is_design = 0)
{
    if ($is_design == 1) {
    ...
    if(!empty($user_rank_info)){
        $_SESSION['rank_name'] = isset($user_rank_info['rank_name']) ? $user_rank_info['rank_name'] : $_SESSION['rank_name'];
        $_SESSION['discount'] = isset($user_rank_info['discount']) ? $user_rank_info['discount'] : $_SESSION['discount'];
        $_SESSION['user_rank'] = isset($user_rank_info['user_rank']) ? $user_rank_info['user_rank'] : $_SESSION['user_rank'];
        $_SESSION['user_id'] = isset($user_rank_info['user_id']) ? $user_rank_info['user_id'] : $_SESSION['user_id'];
    }
 ...
 * @return boolean
 */
function is_guanzhu($supplier_id,$user_id = 0){
    $sql = "select id from ".$GLOBALS['ecs']->table('supplier_guanzhu')." where supplierid = $supplier_id and userid = ".$user_id;
...
//返回订单状态 add by qinglin 2017.09.20
function returnOrderStatus($order_status,$pay_status,$shipping_status){
    // 未确认
    // if ($order_status == 0)  //SB
    // {
        // return 1;
    // }
    // 待付款
    if (in_array($order_status,array(0,1)) and $pay_status == 0 and $shipping_status == 0)
    {
        return 2;
    }
    // 待发货
    if (in_array($order_status,array(1,5,6)) and in_array($shipping_status,array(0,3,5)) and in_array($pay_status,array(2,1)))
    {
        return 3;
    }
    // 待收货
    if (in_array($order_status,array(5,6)) and $shipping_status == 1 and in_array($pay_status,array(1,2)))
    {
        return 4;
    }
    // 已完成
    if (in_array($order_status,array(1,5,6)) and $shipping_status == 2 and $pay_status == 2)
    {
        return 5;
    }
    // 取消
    if ($order_status == 2 and $pay_status == 0 and $shipping_status == 0)
    {
        return 6;
    }
    // 退款
    if ($order_status == 2 and $pay_status == 2 and $shipping_status == 0)
    {
        return 7;
    }
    return 0;
...
//返回订单状态名称 add by qinglin 2017.09.20
function returnOrderStatusName($order=array()){
    if(empty($order)){
        return "";
    }
    // 待付款
    if (in_array($order['order_status'],array(0,1)) && $order['pay_status'] == 0 && $order['shipping_status'] == 0){
        $status_name="待付款";
    }
    // 无效
    elseif ($order['order_status'] == 3 && $order['shipping_status'] == 0 && $order['pay_status'] == 0){
        $status_name="无效";
    }
    // 待发货
    elseif (in_array($order['order_status'],array(1,5,6)) and in_array($order['shipping_status'],array(0,3,5)) and in_array($order['pay_status'],array(2,1))){
        $status_name=($order['extension_code']=='virtual_good')?"待使用":"待发货";
    }
    // 待收货
    elseif (in_array($order['order_status'],array(5,6)) and $order['shipping_status'] == 1 and in_array($order['pay_status'],array(1,2)))
    {
        $status_name="待收货";
    }
    // 已完成
    elseif (in_array($order['order_status'],array(1,5,6)) and $order['shipping_status'] == 2 and in_array($order['pay_status'],array(1,2)))
    {
        $status_name="已完成";
    }
    // 取消
    elseif ($order['order_status'] == 2 and $order['pay_status'] == 0 and $order['shipping_status'] == 0)
    {
        $status_name="已取消";
    }
    // 退款
    elseif ($order['order_status'] == 2 and $order['pay_status'] == 2 and $order['shipping_status'] == 0)
    {
        $status_name="退款";
    }
    // 部分商品待收货【已发货(部分商品)】
    elseif ($order['order_status'] == 6 and $order['shipping_status'] == 4 and $order['pay_status'] == 2)
    {
        $status_name="部分商品待收货";
    }
    else{
...
//所属订单
function returnOrderType($extension_code=''){

    if($extension_code=='group_buy'){ //团购
        $flow_type=1;
    }
    elseif($extension_code=='auction'){ //拍卖
        $flow_type=2;
    }
    elseif($extension_code=='snatch'){ //夺宝奇兵
        $flow_type=3;
    }
    elseif($extension_code=='exchange_goods'){ //积分商城
        $flow_type=4;
    }
    elseif($extension_code=='pre_sale'){ //预售
        $flow_type=6;
    }
    elseif($extension_code=='virtual_good'){ //虚拟
        $flow_type=7;
    }
    elseif($extension_code=='service_goods'){ //服务
        $flow_type=8;
    }
    else{
        $flow_type=0;
...
/**
 * 订单操作
 * handle   [0（查看订单详情）、1（付款、取消订单）、2（申请退款）、3（查看物流、确认收货、申请退款）、4（查看物流、确认收货）、5（查看物流、评价、申请售后）、6（查看物流、申请售后）、7（查看物流、评价）、8（查看物流）、9（删除订单）、10（确认收货、申请退款）、11（确认收货）、12（评价、申请售后）、13（申请售后）、14（评价）]
 */
function returnOrderHandle($order=array()){
    $handle=0;
    // 待付款
    if (in_array($order['order_status'],array(0,1)) && $order['pay_status'] == 0 && $order['shipping_status'] == 0){
        //付款
    ...
    // 待发货
    elseif (in_array($order['order_status'],array(1,5,6)) && in_array($order['shipping_status'],array(0,3,5)) && in_array($order['pay_status'],array(2,1))){
        if(in_array($order['order_status'],array(1,5)) && in_array($order['shipping_status'],array(0,1,3,5))  && $order['pay_status']==2 && $order['back_can']==1){
            //申请退款
            $handle=2;
        }
    }
    // 待收货
    elseif (in_array($order['order_status'],array(5,6)) && $order['shipping_status'] == 1 && in_array($order['pay_status'],array(1,2)) && $order['extension_code']!='virtual_good')
    {


        if($order['order_status']==5 && $order['shipping_status']==1  && $order['pay_status']==2 && $order['back_can']==1){
            // 自提订单不显示物流
            if(!empty($order['pickup_cn']) || $order['extension_code']=='virtual_good'){
                //申请退款（退款或是退货都可)
                //确认收货
                $handle=10;
            }
    ...
    elseif (in_array($order['order_status'],array(1,5,6)) and $order['shipping_status'] == 2 and in_array($order['pay_status'],array(1,2)))
    {
        if($order['order_status']==5 && $order['shipping_status']==2 && $order['pay_status']==2 && $order['extension_code']!='virtual_good' && $order['back_can']==1){
            if($order['can_evaluate']==1){
                // 自提订单不显示物流
                if(!empty($order['pickup_cn']) || $order['extension_code']=='virtual_good'){
                    //评价
                    //申请售后
...
//极光推送
function jpush($body,$extra,$user_id){
	require_once("../qdapi/lib/JPush/JPush.php");
	...
	//$extra = array("type"=>"Goods", "value"=>"667", "info"=>"商品推荐");
	// 简单推送示例
	if($user_id){
		$result = $client->push()
			...
			->addAlias($user_id)
			->addAndroidNotification($body, '', 1, $extra)
			->addIosNotification($body, 'iOS sound', JPush::DISABLE_BADGE, true, 'iOS category', $extra)
 ...
 * 获取图形分类
 */
function graph_type_list() {
    $sql = "SELECT * from ". $GLOBALS['ecs']->table('graph_type') ."  ORDER BY sort_order ASC, type_id ASC";
 ...
 * return array
 */
function list_to_tree($list, $pk = 'type_id', $pid = 'parent_id', $child = '_child', $root=0) {
    // 创建Tree
    $tree = array();
    if(is_array($list)) {
        // 创建基于主键的数组引用
...
// 第三方实名认证接口
function authenticate($card, $real_name, $mobile_phone) {
    ...
    $querys = "id=$card&name=$real_name&telnumber=$mobile_phone";
    $bodys = "";
...
// 第三方获取银行卡信息接口
function get_bank_card($card_number) {
    ...
    $result = json_decode(curl_exec($curl), true);
    if (isset($result['data']['bank_name'])) {
        return $result['data'];
    } else {
        return false;
    }
 ...
 */
function check_bank_card($card_number, $real_name = '', $card = '', $mobile_phone = '') {
    ...
    array_push($headers, "Authorization:APPCODE " . $appcode);
    $real_name = urlencode($real_name);
    $querys = "acct_name={$real_name}&acct_pan={$card_number}&cert_id={$card}&phone_num={$mobile_phone}";
    $bodys = "";
    $url = $host . $path . "?" . $querys;
 ...
 * 发送消息给单个用户
 */
function sent_message($user_id, $msg_type, $title, $message) {
    $rec_arr = array($user_id);
    $rec_arr = json_encode($rec_arr);
    $sql = "INSERT INTO " .$GLOBALS['ecs']->table('member_message'). " (sent_time, title, message, admin_id, sent_all, msg_type) ".
            "VALUES ('".gmtime()."', '".$title."', '" .$message. "', '1', '".$rec_arr."', '" .$msg_type. "')";
    $GLOBALS['db']->query($sql);
    $m_id = $GLOBALS['db']->insert_id();

    // 根据插入数据的id查询需要发送数据的id，和会员信息[多此一步查询，确保数据同步！]
    $sql = 'SELECT `m_id`, `sent_all` FROM '.$GLOBALS['ecs']->table('member_message').' WHERE m_id="'.$m_id.'"';
    ...
    // 根据usersid进行消息的插入
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('mem_mess_list'). ' (mem_id, mess_id, receive_time) VALUES ';
 ...
 * 获取广告图
 */
function get_advlist_( $position, $num = 1 )
{
        $arr = array( );
        $sql = "select ad.ad_id,ad.ad_name,ad.ad_code,ad.ad_link from ".$GLOBALS['ecs']->table( "ad_position" )." as ap left join ".$GLOBALS['ecs']->table( "ad" )." as ad on ad.position_id = ap.position_id where ap.position_name='".$position.( "' and UNIX_TIMESTAMP()>ad.start_time and UNIX_TIMESTAMP()<ad.end_time and ad.enabled=1 limit ".$num );
        if($num>1){
            $res = $GLOBALS['db']->getAll( $sql );
            ...
            $arr['name'] = $res['ad_name'];
            $arr['ad_link'] = $res['ad_link'];
            $arr['image'] = "/data/afficheimg/".$res['ad_code'];
        }
        return $arr;
}
 ...
 * @param  [type] $be_user_id 被关注用户ID
 */
function get_follow_status($user_id, $be_user_id) {
    if ($user_id) {
        $u_to_author = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('user_attention') . " WHERE be_user_id = ".$be_user_id." AND user_id = " . $user_id);
        $author_to_u = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('user_attention') . " WHERE be_user_id = ".$user_id." AND user_id = " . $be_user_id);
        $follow_status = $u_to_author ? ($author_to_u ? USER_ATTENTION_BOTH : USER_ATTENTION) : USER_ATTENTION_NO;
    } else {
        $follow_status = 0;
    }
...
function commission($order_id = 0) {
    if (!empty($order_id)) {
        ...
        if ($order_info['is_design'] == 0) {
            $sql = "SELECT a.goods_id, a.goods_number, b.user_id, b.goods_rank, b.commission1, b.commission2, b.shop_price, b.goods_status FROM " . $GLOBALS['ecs']->table('order_goods') . " a LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " b ON a.goods_id = b.goods_id WHERE a.order_id = '$order_id'";
            $goods_list = $GLOBALS['db']->getAll($sql);
            foreach ($goods_list as $key => $value) {
                // 获取等级数据
                $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_id = '$value[goods_rank]'";
                $rank_info = $GLOBALS['db']->getRow($sql);

                // 第一阶段购买数量（0.4,0.6）
                $level_one_number = floor($rank_info['sale_number'] * 0.4);
                
                // 判断购买件数
                $sql = "SELECT SUM(goods_number) FROM " . $GLOBALS['ecs']->table('order_goods') . " WHERE goods_id = '$value[goods_id]'";
                $total_number = $GLOBALS['db']->getOne($sql);

                // 第一阶段佣金
                if ($total_number > 0 && $total_number <= $level_one_number) {
                    $commission = $value['commission1'];
                }

                // 第二阶段佣金
                if ($total_number > 0 && $total_number > $level_one_number) {
                    $commission = $value['commission2'];
                }
