...
function update_address($address)
{
    ...
    //获取是否已设置默认地址
    $sql = "SELECT address_id FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '".$address['user_id']."'";
    $u_addr = $GLOBALS['db']->getOne($sql);
    ...
    //如果之前不存在默认地址，则把当前地址设置为默认地址
    if(!$u_addr){
        $address['is_default'] = 1;
    }

    if (isset($address['is_default']) && $address['is_default'] > 0 && isset($address['user_id']))
    {
        $sql = "UPDATE ".$GLOBALS['ecs']->table('users') .
 ...
 * @param   int         $is_used         是否已使用 0 为全部  1为未使用 2为已过期  3为已使用  4为未开始       add to qinglin 2017.09.06
 * @return  array       $arr             红保列表
 */
function get_user_bouns_list($user_id, $num = 10, $start = 0, $suppid=-1,$is_used = 0)
{
   /* $sql = "SELECT u.bonus_sn,u.supplier_id, u.order_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ".
    ...
    if($is_used==1){ //未使用
        $sql .= " AND u.order_id=0 AND b.use_start_date<=".$day[0]." AND b.use_end_date>=$cur_date";
    }

    if($is_used==2){ //已过期
        $sql .= " AND u.order_id=0 AND  b.use_end_date<$cur_date";
    }
    ...
    if($is_used==4){ //未开始
        $sql .= " AND u.order_id=0 AND b.use_start_date>".$day[0]."";
    }
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (empty($row['order_id']))
        {
            ...
            {
                $row['is_used'] = 4;//未开始
                $row['status'] = $GLOBALS['_LANG']['not_start']; //未开始
            }
            else if ($row['use_end_date'] < $cur_date)
            {
                $row['is_used'] = 2;
                $row['status'] = $GLOBALS['_LANG']['overdue']; //已过期
            }
            else
            {
                $row['is_used'] = 1;
                $row['status'] = $GLOBALS['_LANG']['not_use']; //未使用
            }
        }
        else
        {
            $row['is_used'] = 3;
            $row['status'] = $GLOBALS['_LANG']['had_use']; //已使用
        }
...
function get_user_bouns_count($user_id, $suppid=-1,$is_used = 0)
{
    $day = getdate();
    $cur_date = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
    ...
    $where="";
    if($suppid>-1){
        $where .= " AND u.supplier_id=".intval($suppid);
    }

    if($is_used==1){ //未使用
        $where .= " AND u.order_id=0 AND b.use_start_date<=".$day[0]." AND b.use_end_date>=$cur_date";
    }
    ...
    if($is_used==4){ //未开始
        $where .= " AND u.order_id=0 AND b.use_start_date>".$day[0]."";
    }
    $sql = "SELECT count(*) ".
           " FROM " .$GLOBALS['ecs']->table('user_bonus'). " AS u ,".
           $GLOBALS['ecs']->table('bonus_type'). " AS b".
           " WHERE u.bonus_type_id = b.type_id AND u.user_id = '" .$user_id. "' $where";
