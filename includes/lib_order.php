...
function available_shipping_list($region_id_list,$suppid=0,$is_default_show=1,$shipping_code=null)
{
    ...
    $sql = 'SELECT DISTINCT s.shipping_id, s.shipping_code, s.shipping_name, ' .
                's.shipping_desc, s.insure, s.support_cod, a.configure , s.support_pickup, s.supplier_id ' .
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' .
                $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' .
                $GLOBALS['ecs']->table('area_region') . ' AS r ' .
            'WHERE r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 ' . $where . ' ORDER BY s.shipping_type asc,s.shipping_order desc';
 ...
 * @return  array   配送方式数组
 */
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false, $is_virtual = 0,$where = '')
{
...
{
    /* 计算订单各种费用之和的语句 */
    $total_fee = " (goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount - integral_money - bonus) AS total_fee ";
    ...
    /* 格式化金额字段 */ //SUM(o.goods_amount  + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee - o.discount) AS o.order_amount,
    if ($order)
    {
 ...
 * @param   integer $flow_type  购物类型 add 2018.01.29 砍价、拼团用
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $user_rank_info = array(), $flow_type = 0, $extendArray = array(), $is_design = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
                ...
                "g.goods_number, g.is_alone_sale,g.exclusive, g.is_shipping,g.goods_type,".
    ...
    /* 是否正在销售 */
    if ($goods['is_on_sale'] == 0 && $is_design == 0)
    {
    ...
    if ($GLOBALS['_CFG']['use_storage'] == 1)
    {
        ...
        {
            $GLOBALS['err']->add(sprintf('对不起，您购买的商品库存不足！', $goods['goods_number']), ERR_OUT_OF_STOCK);
        ...
        /*if (is_spec($spec) && !empty($prod))
        {
            if (!empty($spec))
            {
                ...
                {
                    $GLOBALS['err']->add(sprintf('对不起，您购买的商品库存不足！', $product_info['product_number']), ERR_OUT_OF_STOCK);
        ...
        //检测属性库存是否有设置（diy不用判断属性库存，先注释掉）
        /*if($goods['goods_type'] >0){
            //查询是否只有唯一属性，只有唯一属性的，只读总库存数量就行
            $sql = "SELECT count(*) as num FROM " . $GLOBALS['ecs']->table('attribute') . " WHERE cat_id = '".$goods['goods_type']."' and attr_type > 0";
            ...
            if($attribute_number){
                $is_goods_number = isset($attr_number) ? $attr_number : 0;
            }else{
                $is_goods_number = isset($attr_number) ? $attr_number : $goods['goods_number'];
            }
            if ($num > $is_goods_number)
            {
                $GLOBALS['err']->add(sprintf('对不起，您购买的商品库存不足！', $product_info['product_number']), ERR_OUT_OF_STOCK);
                return false;
            }
    ...
    $goods_price            = get_final_price($goods_id, $num, true, $spec, '', $is_design);
    $goods['market_price'] += $spec_price;
    $goods_attr             = get_goods_attr_info($spec);
    $goods_attr_id          = join(',', $spec);
    //print_r($goods['market_price']);
    /* 初始化要插入购物车的基本件数据 */
    $parent = array(
        ...
        'is_design'     => $is_design,//1位设计库商品,不显示在前端购物车
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
               $GLOBALS['err']->add(sprintf('对不起，您购买的商品已库存不足！', $num), ERR_OUT_OF_STOCK);
        ...
        else //购物车没有此物品，则插入
        {
            ...
            $parent['parent_id']    = 0;
            // file_put_contents('test.txt', var_export($parent, true));
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart'), $parent, 'INSERT');
        }
        ...
        */
        ...
        if($flow_type == 101){
            //获取商品最新砍价值
            $log_info = $GLOBALS['db']->getRow("SELECT bargain_id,now_price,product_id FROM " . $GLOBALS['ecs']->table('bargain_log') . " WHERE goods_id = '$goods_id' and product_id = '".$product_info['product_id']."' and help_user_id = '".$_SESSION['user_id']."' and status = 0 order by bargain_id desc,now_price asc");
            //如果没有就读取商品原价
            $now_price = isset($log_info['now_price']) ? $log_info['now_price'] : $goods_price;
            //更新砍价活动
            $GLOBALS['db']->query("update ".$GLOBALS['ecs']->table('cart')." set rec_type='101',extension_id = '".$log_info['bargain_id']."' ,extension_code = 'bargain',goods_price='$now_price' WHERE user_id = '".$_SESSION['user_id']."' AND goods_id = '$goods_id' ");
        }
        ...
        if($flow_type == 102){
            ...
            if (!empty($spec))
            {
                $spec_price   = spec_price($spec);//属性价格
                $shop_price += $spec_price;
            }
            //计算拼团折扣价
            $now_price = round($group_info['group_discount'] * $shop_price,2);
            //更新拼团活动
            $GLOBALS['db']->query("update ".$GLOBALS['ecs']->table('cart')." set rec_type='102',extension_id = '".$group_info['id']."' ,extension_code = 'group',goods_price='$now_price',group_log_id='".$extendArray['group_log_id']."' WHERE user_id = '".$_SESSION['user_id']."' AND goods_id = '$goods_id' ");
        }
 ...
 * @param   string  $other  需要扩展的条件字符串
 */
function clear_cart_goods($goods_id=0)
{
 ...
 * @return  array   红包数组
 */
function user_bonus($user_id, $goods_amount = 0,$supplier_id=0)
{
    $day    = getdate();
    ...
    if($goods_amount){
        ...
        {
            $sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id, b.bonus_sn, b.order_id,t.min_goods_amount, t.use_start_date, t.use_end_date, t.supplier_id " .
 ...
 * @return  array
 */
function get_consignee($user_id,$address_id=0)
{
    if($address_id>0){
        $arr = array();
 ...
 * @return  bool
 */
function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS,$user_id=0)
{
    ...
    if ($order_id <= 0)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('cart') .
                " WHERE user_id = '$user_id' AND is_real = 1 " .
                "AND rec_type = '$flow_type'";
    }
    else
    {
...
function check_consignee_info($consignee, $flow_type, $user_id=0)
{

    if (exist_real_goods(0, $flow_type, $user_id))
    {
        /* 如果存在实体商品 */
...
/**
 * 获取查询条件 add by qinglin 2017.09.18
 * @param   string  $alias  order表的别名（包括.例如 o.）
 */
function order_query_sql2($status, $alias = ''){
    if(!$status){
        return '';
    }
    // 未确认
    ...
    if ($status == 2)
    {
        //order_status(0,1)  pay_status(0) shipping_status(0)
        return " AND {$alias}order_status " . db_create_in(array(OS_UNCONFIRMED, OS_CONFIRMED)) .
               " AND {$alias}pay_status = '" . PS_UNPAYED . "'" .
               " AND ({$alias}shipping_status = '" .SS_UNSHIPPED. "' " . " OR {$alias}pay_id " . db_create_in(payment_id_list(false)) . ") ";
    }
    // 待发货
    if ($status == 3)
    {
        //order_status(1,5,6)  pay_status(2,1) shipping_status(0,3,5)
        return " AND   {$alias}order_status " .
                 db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) .
               " AND   {$alias}shipping_status " .
                 db_create_in(array(SS_UNSHIPPED, SS_PREPARING, SS_SHIPPED_ING)) .
    ...
    if ($status == 4)
    {
        //order_status(5,6)  pay_status(2,1) shipping_status(1)
        return " AND   {$alias}order_status " .
                 db_create_in(array(OS_SPLITED, OS_SPLITING_PART)) .
    ...
    if ($status == 5)
    {
        //order_status(1,5,6)  pay_status(2) shipping_status(2)
...
//获得商家是否可开发票及发票类型
function get_supplier_fapiao_list($suppid=0,$flow_type,$goods_price=0)
{
    $fapiao['can_invoice']=0;


    if($suppid>0){
        $can_invoice = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['ecs']->table('supplier_shop_config') . " WHERE code='can_invoice' AND supplier_id = " . $suppid);
        $invoice_type = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['ecs']->table('supplier_shop_config') . " WHERE code='invoice_type' AND supplier_id = " . $suppid);
    }
    else{
        $can_invoice = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['ecs']->table('shop_config') . " WHERE code='can_invoice'");
        $invoice_type = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['ecs']->table('shop_config') . " WHERE code='invoice_type'");
    }
    ...
    $inv_tax_list = array();
    /* 如果能开发票，取得发票内容列表 */
    if ($can_invoice == '1' && $flow_type != CART_EXCHANGE_GOODS)
    {

        //$inv_type_list = array();
        foreach ($invoice_type['type'] as $key => $type)
        {
            if (!empty($type)&&$invoice_type['enable'][$key]=='1')
            {
                //$inv_type_list[$type] = $GLOBALS['_LANG'][$type] . ' [' . floatval($invoice_type['rate'][$key]) . '%]';
                $inv_type_list[$type] = $GLOBALS['_LANG'][$type];
                ...
                $inv_rate_list[$type] = $rate;//发票税率
                $inv_tax_list[$type] = $tax;//发票税费

            }
        }

        if(!empty($inv_type_list)){
            $fapiao['can_invoice']=1;
        }

    }
    $fapiao['invoice_type']=$inv_type_list;
    //$fapiao['invoice_rate']=$inv_rate_list;//发票类型税率
    //$fapiao['invoice_tax']=$inv_tax_list;//发票类型税费
