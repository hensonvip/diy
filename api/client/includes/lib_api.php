<?php
    ...
    function API_AddBrand($post)
    {
        ...
        if (!$is_only)
        {
         ...
         /* 处理图片 */
        $brand_logo_data = filter_var($_POST['brand_logo'],FILTER_SANITIZE_STRING);
        $img_name = upload_image($brand_logo_data, 'brandlogo');
        if($img_name !== false)
        {
            $img_name = basename($img_name);
        }
    ...
    function API_AddGoods($post)
    {
        ...
        */
        ...
        {
            $img_url_data = filter_var_array($_POST['img_url'],FILTER_SANITIZE_STRING);//新增过滤
            if(empty($img_url_data)){
                $img_url_data = filter_var($_POST['img_url'],FILTER_SANITIZE_STRING);//新增过滤
            }
            foreach ($img_url_data as $key => $img_url)
            {
