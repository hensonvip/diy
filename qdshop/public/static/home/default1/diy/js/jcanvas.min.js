...
// Measures canvas text
    function _measureText(canvas, ctx, params, lines) {
        ...
        for(var is=0;is<lines.length;is++){
            var ss=ctx.measureText(lines[is]).width;
            if(bb<ss){
                numis=is;
                bb=ss;
            }
        }
        params.width = bb+params.ls*(lines[numis].length-1)*parseInt(params.fontSize);
        ...
        if (propCache.text === params.text && propCache.fontStyle === params.fontStyle && propCache.fontSize === params.fontSize && propCache.fontFamily === params.fontFamily && propCache.maxWidth === params.maxWidth && propCache.lineHeight === params.lineHeight) {
            ...
            params.height =parseInt(params.fontSize)  * lines.length * params.lineHeight;
                ...
                if (params.visible) {
                    ...
                    if (params.radius) {
                        ...
                        for (l = 0; l < lines.length; l += 1) {
                            ...
                            var po=x;
                            var cd=0;
                            var fonts=parseInt(params.fontSize);
                            var ls=fonts*params.ls;
                            var hd=Math.ceil(fonts/20);
                            var ss=params.lineHeight*fonts;
                            var thisy=params.y-params.height/2+ss*l+ss/2+fonts/2;
                            for(var xqs=0;xqs<line.length;xqs++){
                                var sizesd_1=ctx.measureText(line[xqs]).width;
                                if(xqs!=line.length-1){
                                    cd+=sizesd_1+ls;
                                }else{
                                    cd+=sizesd_1;
                                }
                            ...
                            if(navigator.userAgent.indexOf('Firefox')>=0){
                                y=y+Math.ceil(fonts/9)
                            }
                            if(params.align=='left'){
                                ctx.beginPath();
                                ctx.moveTo(po,thisy-fonts/2+hd);
                                ctx.lineTo(po+cd, thisy-fonts/2+hd);
                                ctx.lineWidth=fonts;
                                ctx.stroke();
                                ctx.closePath();
                                if(params.underLine){ctx.fillRect(po,thisy,cd,hd);}
                                for(var num=0;num<line.length;num++){
                                    if(num!=0){
                                        var sizesd=ctx.measureText(line[num-1]).width;
                                        po=po+sizesd+ls;
                                    }
                                    ctx.fillText(line[num], po, y);
                                    // if (params.strokeWidth !== 0) {
                                    //     ctx.strokeText(line[num], po, y)
                                    //     // Only stroke if the stroke is not 0
                                    // }
                                }

                            }else if(params.align=='right'){
                                var indexns=line.length-1;
                                var pos=po;
                                for(var nums=0;nums<line.length;nums++){
                                    if(indexns!=line.length-1){
                                        var sizesds=ctx.measureText(line[indexns+1]).width;
                                        pos=pos-sizesds-ls;
                                    }
                                    if(indexns==0){
                                        pos=pos-ctx.measureText(line[0]).width;
                                        ctx.beginPath();
                                        ctx.moveTo(pos,thisy-fonts/2+hd);
                                        ctx.lineTo(pos+cd, thisy-fonts/2+hd);
                                        ctx.lineWidth=fonts;
                                        ctx.stroke();
                                        ctx.closePath();
                                        break;
                                ...
                                var indexn=line.length-1;
                                for(var num=0;num<line.length;num++){
                                    if(indexn!=line.length-1){
                                        var sizesd=ctx.measureText(line[indexn+1]).width;
                                        po=po-sizesd-ls;
                                    }
                                    ctx.fillText(line[indexn], po, y);
                                    // if (params.strokeWidth !== 0) {
                                    //     ctx.strokeText(line[indexn], po, y)
                                    //     // Only stroke if the stroke is not 0
                                    // }
                                    if(indexn==0){
                                        po=po-ctx.measureText(line[0]).width;
                                        break;
                                    }
                                    indexn--;
                                }
                                ...
                                ctx.closePath();
                                if(params.underLine){ctx.fillRect(po,thisy,cd,hd);}
                                for(var num2=0;num2<line.length;num2++){
                                    if(num2!=0){
                                        var sizesd2=ctx.measureText(line[num2-1]).width;
                                        po=po+sizesd2+ls;
                                    }
                                    var wylxl=po+ctx.measureText(line[num2]).width/2;
                                    ctx.fillText(line[num2], wylxl, y);

                                    // if (params.strokeWidth !== 0) {
                                    //     ctx.strokeText(line[num2], wylxl, y)
                                    //     // Only stroke if the stroke is not 0
                                    // }
                                }
                            }
