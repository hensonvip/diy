...
// 编辑时素材图片还原
// mD.imgBox.append('<li data-load="true" data-name="file_id" class="imgBoxItem"><b style="display:none"><strong class="loader-inner"></strong><strong class="loader-outter"></strong></b><span  style="display:none"></span><i>X</i><em style="background-image: url('+file_url+')"></em></li>');
// mD.imgData.push({name:ret.data.file_id,data:'/'+ret.data.file_url});
...
$(document).on('click', '.cutImgNo', function(event) {
...
$(".addImgFile").change(function() {
    ...
    reader.onload = function() {
        ...
        var go=false;
        mD.imgBox.find('li').each(function () {
            if($(this).attr('data-src')==result){
                go=true;
                return false;
            }
        });
        ...
        //             mD.imgBox.append('<li data-name="'+mD.imgData[x].name+'" class="imgBoxItem"><b>上传中</b><span>重新上传</span><i>X</i><em style="background-image: url('+this.result+')"></em></li>')
        //         }
        //         return false;
        //     }
        // }
        mD.imgBox.append('<li data-load="false" data-src="'+result+'" data-get="'+name+'"  class="imgBoxItem"><b><strong class="loader-inner"></strong><strong class="loader-outter"></strong></b><span></span><i>X</i><em style="background-image: url('+result+')"></em></li>');
        var li= $('.imgBoxItem[data-get='+name+']');
        ajaxUimg(li);
        mD.addImgNum++;
    };
...
function ajaxUimg(li) {
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: {file: li.attr('data-src')}
    })
    .done(function(ret) {
        if(ret.code == '200'){
                li.find('b').hide();
                li.attr('data-name',ret.data.file_id).attr('data-load','true');
                mD.imgData.push({name:ret.data.file_id,data:'/'+ret.data.file_url});
            }else{
                li.find('span').show();
            }
...
function ajaxRimg(li) {
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: {file_id: li.attr('data-name')}
    })
    .done(function(ret) {
        if(ret.code == '200'){
                var ob=getObj(mD.imgData,'name',li.attr('data-name'),true);
...
mD.imgBox.on('click','li',function (e) {
    ...
    else if(target.closest("i").length != 0){
...
$(document).on('click', '.xgset', function(event) {
...
$(document).on('click', '#fz .x', function(event) {
    if(mD.actName=='none'){
        return false;
    }
    cxD();
...
$(document).on('click', '#fz .s', function(event) {
    if(mD.actName=='none'){
        return false;
    }
    cxD();
...
$(document).on('click', '#fz .t', function(event) {
    if(mD.actName=='none'){
        return false;
    }
    cxD();
...
$(document).on('click', '#fz .b', function(event) {
    if(mD.actName=='none'){
        return false;
    }
    cxD();
...
mD.choT.find('span').click(function () {
    ...
    s.removeClass('on').css({'background-image':'url('+s.attr('data-img')+')'});*/
    mD.choT.find('span').each(function(index, el) {
        $(this).removeClass('on').css({'background-image':'url('+$(this).attr('data-img')+')'});
    });
    $(this).addClass('on').css({'background-image':'url('+$(this).attr('data-imgon')+')'});
...
function initOne() {
    ...
    typeTel.addClass('on').css({'background-image':'url('+typeTel.attr('data-imgon')+')'});
...
$(document).on('click', '#sx', function(e) {
    jx(e);
});
...
$(document).on('click', '.ylbtn .b1', function(event) {
...
/*创意擂台左边预览作品*/
$(document).on('click', '.ylbtn .b3', function(event) {
    var h=mD.allData[0].height;
    mD.allData[1].strokeStyle='transparent';
    for(var is=0;is<mD.allData.length;is++){
        if(mD.allData[is].typeName=='mb'){
    ...
    setCanvasSize(900);
    $('.mep_tleft img').attr('src',mD.canvas.getCanvasImage('png'));
    mD.allData[1].strokeStyle='#ccc';
    for(var is2=0;is2<mD.allData.length;is2++){
        if(mD.allData[is2].typeName=='mb'){
            mD.allData[is2].fillStyle='rgba(0,0,0,0.1)'
...
$(document).on('click', '.ylImg i em', function(event) {
    $('.ylImg').hide();
       ...
       var e = event || window.event || arguments.callee.caller.arguments[0];
        if(e){
            ...
            if(!e.ctrlKey&&!e.shiftKey){
                    if((code==187||code==61||code==107)&&mD.moveb.is(':hidden')){ // 按 +

                        $('#fd').click();
                    }
                    if((code==189||code==173||code==109)&&mD.moveb.is(':hidden')){ // 按 -
                        $('#sx').click();
                    }
...
$(window).load(function () {
    ...
    $(document).on('click', '.cart_wrap .bar', function(event) {
    ...
    //切换浏览里的款式颜色
    $('.mprev_type label input').change(function() {
    ...
    $(document).on('change', '.cart_wrap label input', function() {
        var _this = $(this);
        if (_this.parents('.cr_b').hasClass('size_box')) {
            _this.parents("label").addClass('on').siblings().removeClass('on');
        } else {
            var index = layer.load(0, {
                shade: [0.1,'#fff']
            });
            var goods_attr_id = _this.parents('.mep_tright').find('.sex_box input:checked').val();
            var goods_attr_id2 = _this.parents('.mep_tright').find('.color_box input:checked').val();
            $.ajax({
                url: get_goods_img,
                type: 'GET',
                dataType: 'json',
                data: {goods_attr_id: goods_attr_id, goods_attr_id2: goods_attr_id2},
                success: function(res) {
                    layer.close(index);
                    if (res.code == 200) {
                        if (_this.parents('.cr_b').hasClass('sex_box')) {
                            _this.parents('label').css('background-image','url('+_this.parents("label").attr("data-srcon")+')').addClass('on').siblings().removeClass('on').each(function(argument) {
                               $(this).css('background-image','url('+$(this).attr("data-src")+')');
                            });
                        } else {
                            _this.parents("label").addClass('on').siblings().removeClass('on');
                        }
                        _this.parents('.cart_box').children('.cart_pic').children('img').attr('src', web_url+res.data.goods_img);
                    } else {
                        _this.prop('checked', false).parents('label').siblings('label').children('input').prop('checked', true);
    ...
    $(window).resize(function(argument) {
        $('.cart_wrap.type2 .t2_wrap').mCustomScrollbar("destroy");
    ...
    //添加多件
    $(document).on('click', '.t2_wrap .a_btn', function() {
        var base_html = "<div class='cart_box'>" + $(this).parents('.cart_box').html() + "</div>";

        $('.cart_wrap.type2 .t2_wrap').mCustomScrollbar("destroy");
        $('.box2 .t2_wrap').prepend(base_html);
        $(this).parents('.cart_box').find('label.on').each(function() {
        ...
        // 失效后重新设置第一个为选择
        $('.box2 .cart_box').eq(0).find("input[type='radio']").each(function() {
            var name = $(this).attr('name');
            var new_name = name + '_' + id;
            $(this).attr('name', new_name);
            // $(this).parents('.sex_box').find('input:checked').parent('label').addClass('on').siblings('label').removeClass('on');
            // $(this).parents('.color_box').find('input:checked').parent('label').addClass('on').siblings('label').removeClass('on');
            // $(this).parents('.size_box').children('label').eq(0).addClass('on').siblings('label').removeClass('on');
        });
         $('.box2 .cart_box').eq(0).find('label.on').each(function() {
            $(this).children('input').prop('checked', true)
        });
    ...
    //删除多件
    $(document).on('click', '.t2_wrap .m_btn', function() {
        var _this = $(this);
        layer.confirm('确定要删除定制好的T恤？', {
            title: false,
            closeBtn: 0,
            btn: ['取 消','确 定'] //按钮
        }, function(){
    ...
    $(document).on('click', '.ylbtn .b2,.b3', function(event) {
        var sub_b2 = $(this).attr('class');
        var ss = layer.load(0, {
            shade: [0.1, '#fff']
        });
        var ary=[];
        var h=mD.allData[0].height;
        var src=mD.allData[0].source;
        mD.allData[1].strokeStyle='transparent';
        for(var is=0;is<mD.allData.length;is++){
            if(mD.allData[is].typeName=='mb'){
                mD.allData[is].fillStyle='transparent'
        ...
        mD.choTcolor.find('li').each(function(){
            var _this=$(this);
            mD.allData[0].source=$('.Timg .'+_this.attr('data-src'))[0];
            setCanvasSize(900);
            var file=mD.canvas.getCanvasImage('png');
            var l=0
        ...
        var sjt=c.toDataURL("image/png");

        if($(this).attr('class') =='b3'){
    ...
    //     $(this).find('dl,.snav_main').stop(true,true).slideUp(300);
    // });
    ...
    // });
    var type = 0;
    $(document).on('click', '.hcart_con dd .box5 .del', function(event) {
    ...
    $('.c_tip').hover(function(argument) {
        var that=this;
        layer.tips($(this).attr('data-tip'), that,{
            time:1000,
            anim: 5,
            tips: [1, '#fff']
        });
