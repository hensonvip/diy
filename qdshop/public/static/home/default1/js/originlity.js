...
//获取数据
 function getPopup(result,ex){
    if(ex.length>0){
        if($('.pr .cylt_bar').find('li.on a').text() == '已通过'){
             $('#already_box .ipro_popup').empty().append(result);
        }else if($('.pr .cylt_bar').find('li.on a').text() == '待审核'){
             $('#wait_box .ipro_popup').empty().append(result);  
        }else if($('.pr .cylt_bar').find('li.on a').text() == '未通过'){
             $('#not_box .ipro_popup').empty().append(result);
        }
    }else{
        $('.ipro_popup').empty().append(result);
    }
 }

//我的列表关闭其他分类弹窗
function closePopup(ex){
    if(ex.length>0){
        if($('.pr .cylt_bar').find('li.on a').text() == '已通过'){
             $('#wait_box .ipro_popup').css('display','none');
             $('#not_box .ipro_popup').css('display','none');
        }else if($('.pr .cylt_bar').find('li.on a').text() == '待审核'){
             $('#already_box .ipro_popup').css('display','none');
             $('#not_box .ipro_popup').css('display','none'); 
        }else if($('.pr .cylt_bar').find('li.on a').text() == '未通过'){
             $('#already_box .ipro_popup').css('display','none');
             $('#wait_box .ipro_popup').css('display','none');
...
$('.ipro_popup').scroll(function () {
    ...
    if (iscrollTop >= iHeight - $(window).height() * 1.2 && c_load == false) {
        ...
        var count = $(".ipro_com_bot_pf").text().replace(/[^0-9]/ig,"");
        c_load = true;
        console.log(count);
        //console.log($('.ipro_com_bot_ul li').length);
