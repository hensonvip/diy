...
$(function(){
    ...
    $('.cylt2_plist li .name').click(function(e) {
        e.stopPropagation();
    });

/*    $('body').on('click','.cylt2_plist.tp li .tp_box',function(e) {
        e.stopPropagation();
...
/*    $('body').on('click','.ipd_tp',function() {
        if($(this).hasClass('tp_ed')){ return false; }
        var number = $('.ipd_user .price.ps .piaoshu').text();
        $(this).addClass('tp_ed disabled').html("已投票");
        $('.ipd_user .price.ps').addClass('tp_ed');
        $('.ipd_user .price.ps .piaoshu').text(parseInt(number) + 1);
    });*/

/*    $('.ipd_tp').click(function() {
        if($(this).hasClass('tp_ed')){ return false; }
        var number = $('.ipd_user .price.ps .piaoshu').text();
        var record = $(this)
        $.ajax({
            type: 'GET',
            url: "{:url('Originality/vote_ajax')}",
            data: {vote:vote, },
            dataType: 'json',
            success: function (result) {
                $(this).addClass('tp_ed disabled').html("已投票");
...
//回复 评论 点赞
document.write("<script src='__DIYPC__/js/diy_details_public.js'></script>");
/*if ($(this).parents('.ipro_zan').hasClass('on')) {
            $(this).parents('.ipro_zan').removeClass('on');
        } else {
            $(this).parents('.ipro_zan').addClass('on');
...
//提交回复
$('#reply').find(".ipro_com_bot_sub").click(function(){
    var _tihs = $(".ipro_wrap");
    ...
    $.ajax({
        type: 'POST',
        url: reply,
        data: {user_id:user_id,content:content,record_id:record_id,comment_id:comment_id},
        dataType: 'json',
        success: function (result) {
...
//提交评论
$('#comment').find(".ipro_com_bot_sub").click(function(){
    var _tihs = $(".ipro_wrap");
    ...
    $.ajax({
        type: 'POST',
        url: comment,
        data: {author_id:author_id,content:content,record_id:record_id,diy_id:diy_id},
        dataType: 'json',
        success: function (result) {
            if(result['user_data']['code'] == 1){
                layer.msg("评论成功");
