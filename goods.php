...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
		...
		$smarty->assign('pictures',            get_goods_gallery_attr_hunuo_com($goods_id, $goods_attr_id)); // 商品相册
...
$goods_n=$db->getOne('SELECT goods_number FROM' . $ecs->table('goods') . " WHERE goods_id = '$id'");
