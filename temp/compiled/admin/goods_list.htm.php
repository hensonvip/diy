...
<!-- 商品列表 -->
<form method="post" action="" name="listForm" onsubmit="return confirmSubmit(this)" class="layui-form">
  <!-- start goods list -->
...
<table cellpadding="3" cellspacing="1" class="layui-table">
  <tr>
	...
	<?php endif; ?>
    <th style="max-width:600px;"><a href="javascript:listTable.sort('goods_name'); "><?php echo $this->_var['lang']['goods_name']; ?></a><?php echo $this->_var['sort_goods_name']; ?></th>
	...
	<th><a href="javascript:listTable.sort('exclusive'); ">手机专享价</a><?php echo $this->_var['sort_shop_price']; ?></th>
...
?>
  <tr>
	...
	<td align="right"><span onclick="listTable.edit(this, 'edit_exclusive', <?php echo $this->_var['goods']['goods_id']; ?>)"><?php echo $this->_var['goods']['exclusive']; ?></span></td>
    ...
    <?php endif; ?>
    <td align="center"><a href="goods_tag.php?act=list&goods_id=<?php echo $this->_var['goods']['goods_id']; ?>">标签</a></td>
    <td align="center"><?php if ($this->_var['goods']['goods_status'] == 0): ?>后台添加<?php elseif ($this->_var['goods']['goods_status'] == 1): ?>diy商品未出售<?php elseif ($this->_var['goods']['goods_status'] == 2): ?>diy商品申请出售审核中<?php elseif ($this->_var['goods']['goods_status'] == 3): ?>审核不通过<?php elseif ($this->_var['goods']['goods_status'] == 4): ?>diy商品已出售<?php endif; ?></td>
    <td align="center">
      <?php if ($this->_var['goods']['goods_status'] == 2): ?>
        <a href="goods.php?act=check&type=yes&id=<?php echo $this->_var['goods']['goods_id']; ?>" title="通过审核"><img src="images/icon_yes.gif" width="16" height="16" border="0"></a>
        <a href="goods.php?act=check&type=no&id=<?php echo $this->_var['goods']['goods_id']; ?>" title="不通过审核"><img src="images/icon_no.gif" width="16" height="16" border="0"></a>
      <?php elseif ($this->_var['goods']['goods_status'] == 3): ?>
        <a href="goods.php?act=check&type=yes&id=<?php echo $this->_var['goods']['goods_id']; ?>" title="通过审核"><img src="images/icon_yes.gif" width="16" height="16" border="0"></a>
      <?php elseif ($this->_var['goods']['goods_status'] == 4): ?>
        <a href="goods.php?act=check&type=no&id=<?php echo $this->_var['goods']['goods_id']; ?>" title="不通过审核"><img src="images/icon_no.gif" width="16" height="16" border="0"></a>
      <?php endif; ?>
...
<div style="height:50px;">
	<span style="float:left;line-height:35px;">操作：</span>
    <input type="hidden" name="act" value="batch" />
    <select name="type" id="selAction" lay-filter="sel_selAction">
    ...
    <div style="display:none" id="target_cat_box">
        <select name="target_cat" lay-filter="sel_changeAction">
            <option value="0"><?php echo $this->_var['lang']['select_please']; ?></option><?php echo $this->_var['cat_list']; ?>
        </select>
    </div>
	<?php if ($this->_var['suppliers_list'] > 0): ?>
    <div style="display:none" id="suppliers_id_box">
        <!--二级主菜单：转移供货商-->
        <select name="suppliers_id" lay-filter="sel_changeAction">
...
?>
  ...
  function change_this(val)
  {
    ...
    document.getElementById("target_cat_box").style.display = frm.elements['type'].value == 'move_to' ? '' : 'none';
			
	<?php if ($this->_var['suppliers_list'] > 0): ?>
        //frm.elements['suppliers_id'].style.display = frm.elements['type'].value == 'suppliers_move_to' ? '' : 'none';
		document.getElementById("suppliers_id_box").style.display = frm.elements['type'].value == 'suppliers_move_to' ? '' : 'none';
	<?php endif; ?>
...
<script type="text/javascript">
    layui.use('form', function(){
        ...
        form.on('select(sel_selAction)', function(data){
            change_this(data.value);
            form.render(); //更新全部
        });  
        form.on('select(sel_changeAction)', function(data){
            changeAction();
            //form.render(); //更新全部
        }); 
    });
