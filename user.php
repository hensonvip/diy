...
$ui_arr = array(
	...
	're_binding_email', 'ch_email', 'ck_email', 'step_1', 'forget_password', 'back_order_detail', 'del_back_order', 'back_order_detail_edit', 'add_huan_goods','myCommentShaidan',
/*余额额支付密码*/
'act_forget_pass', 're_pass', 'auction_list', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code','bank_card_list','add_bank_card'
); // 代码修改
   // By
   // 
/* 余额额支付密码 */
...
function action_login ()
{
	...
	//获取记录的帐号密码
	$smarty->assign('remember_name', $_COOKIE['remember_name']);
	$smarty->assign('remember_pwd', $_COOKIE['remember_pwd']);
...
//AXAJ 登陆修改
	...
	if($_REQUEST['remember'] == 1){
		setcookie('remember_name', $username,time()+7*3600);
		setcookie('remember_pwd', $password,time()+7*3600);
	}else{
		setcookie('remember_name','');//设置过期
		setcookie('remember_pwd', '');//设置过期
	}
...
function action_collection_list ()
{
	...
	$smarty->assign('pager', $pager);
	$smarty->assign('goods_list', get_collection_goods($user_id, 12, $pager['start']));
...
function action_account_raply ()
{
	...
	//银行卡列表
	$sql = "select id,card_name from " . $ecs->table('bank_card') . " order by id asc";
	$bank_card_list = $db->getAll($sql);
	$smarty->assign('bank_card_list', $bank_card_list);
...
/* 显示银行卡列表 */
function action_bank_card_list ()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	/* 获取银行卡的数量 */
	$sql = "SELECT COUNT(*) " . "FROM " . $ecs->table('bank_card') . " WHERE user_id = '$user_id'";
	$record_count = $db->getOne($sql);
	$pager = get_pager('user.php', array(
		'act' => $action
	), $record_count, $page);
	// jx 缺货信息添加商家名称和商品图片
	$bank_card_list = get_bank_card_list($user_id, $pager['size'], $pager['start']);
	foreach($bank_card_list as $key => $value)
	{
		//$sql = "select bank_icon from " . $ecs->table('bank_icon') . " WHERE '".$value['card_name']."' LIKE CONCAT('%',bank_icon_name,'%')";
		//$bank_icon = $db->getOne($sql);
		//$bank_card_list[$key]['bank_icon'] = 'data/bankImg/'.$bank_icon;
	}
	$smarty->assign('bank_card_list', $bank_card_list);
	$smarty->assign('pager', $pager);
	$smarty->display('user_clips.dwt');
 ...
 */
function get_bank_card_list($user_id, $num, $start)
{
    $bank_card_list = array();
    $sql = "SELECT * ".
           "FROM " .$GLOBALS['ecs']->table('bank_card')." WHERE user_id = '$user_id' ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);

    while ($row = $GLOBALS['db']->fetchRow($res))
    {

        $bank_card_list[] = array('id'       => $row['id'],
                           'real_name'   => $row['real_name'],
                           'card_name' => $row['card_name'],
                           'card_short_name' => $row['card_short_name'],
...
/* 添加银行卡页面 */
function action_add_bank_card ()
{
	$smarty = $GLOBALS['smarty'];
	$action = $GLOBALS['action'];

	$bank_list = $GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['ecs']->table('bank') . " order by sort_order ASC,id ASC");	
...
/* 添加银行卡的处理 */
function action_act_add_bank_card ()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$sql = "INSERT INTO " . $GLOBALS['ecs']->table('bank_card') .
            " (user_id, real_name,bank_id, card_number, card_name, card_info, card_type, add_time)".
            " VALUES ('$user_id', '$real_name','$bank_id', '$card_number', '$card_name', ".
            " '$card_info', '$card_type', '$add_time')";
    $GLOBALS['db']->query($sql);
	
	show_message($_LANG['bank_list_success'], $_LANG['back_bank_list'], 'user.php?act=bank_card_list', 'info');
}

/* 默认银行卡 */
function action_act_default_bank_card ()
{
	$user_id = $_SESSION['user_id'];
	...
	$GLOBALS['db']->query($sql);
	//设置默认
	$sql = 'UPDATE ' . $GLOBALS['ecs']->table('bank_card') . " SET is_default = 1 WHERE user_id = '$user_id' and id = '$id'";
	$GLOBALS['db']->query($sql);
	
	ecs_header("Location: user.php?act=bank_card_list\n");
	exit();
}

/* 删除银行卡 */
function action_act_del_bank_card ()
{
	$user_id = $_SESSION['user_id'];
	
	$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
	$sql = 'DELETE FROM ' .$GLOBALS['ecs']->table('bank_card').
...
/* 商品评价and晒单(组合) add by qinglin 2017.09.22 */
function action_myCommentShaidan ()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	if($o_id)
	{
		...
		$sql = "SELECT og.*, o.add_time, o.shipping_time_end, o.order_id, g.goods_thumb, s.shaidan_id, s.pay_points AS shaidan_points, s.status AS shaidan_status,
			...
			LEFT JOIN " . $ecs->table('comment') . " AS c ON og.rec_id=c.rec_id
			LEFT JOIN " . $ecs->table('supplier_shop_config') . " AS ssc ON ssc.supplier_id=g.supplier_id AND ssc.code='shop_name'
			WHERE o.user_id = '$user_id' AND og.rec_id = '$rec_id' AND o.shipping_time_end > 0 AND og.is_back = 0 ORDER BY o.add_time DESC";
	}
	else
	{
		$sql = "SELECT og.*, o.add_time, o.shipping_time_end, o.order_id, g.goods_thumb, s.shaidan_id, s.pay_points AS 	shaidan_points, s.status AS shaidan_status, 
	...
	$smarty->assign('item_list', $item_list);//print_r($item_list);die;

	$smarty->display('user_comment_shaidan.dwt');
}

//提交评论晒单  add by qinglin 2017.09.22 
function action_myCommentShaidanSend ()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$rec_id = intval($_POST['rec_id']);
	$goods_id = intval($_POST['goods_id']);
	$id_value = intval($_POST['goods_id']);
